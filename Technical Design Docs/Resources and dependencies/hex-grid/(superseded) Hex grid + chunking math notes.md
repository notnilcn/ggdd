...
> **⚠️ Superseded — background reading only.** The hand-rolled C# `WorldToHex`/`SmallToBig`/`Hexmod`/`InvHexmod` below were never the implementation: `server/spacetimedb/src/world/hex.rs` delegates that math to the **`hexx` crate** (`to_higher_res`/`to_lower_res`/`from_hexmod_coordinates`), and `chunk_index` is a **hex-spiral** index (`spiral_chunk_index`), not the linear `cr * chunkCols + cq` used here. The `BuildingTile` shape and `GenerateMap` sketch are also stale — see `world/tables.rs` and `world/terrain/`. Kept for the coordinate-system explanations and the reference links.
