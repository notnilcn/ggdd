...
Diagram('conversions-offset-cube', {
    mixins: [mixins.highlight],
...
// NOTE: this is a compound diagram with *six* sub-diagrams. Normally
// a diagram has a single 'highlight' from the highlight mixin. But
// I want the top three sub-diagrams (flat) to not highlight at the
// same time as the bottom three (pointy), so I have an extra
// highlightOrientation field to record the last highlight type.
Diagram('conversions-doubled-cube', {
    mixins: [mixins.highlight],
    data: {
        output: 'axial',
        highlightOrientation: 'flat',
    },
    computed: {
        qgrid() {
            return makeQDoubledRectangularShape(-2, 2, -4, 4);
        },
        rgrid() {
            return makeRDoubledRectangularShape(-4, 4, -2, 2);
        },
        output_constructor() {
            return this.output === 'axial'? "Hex(q, r)" : "Cube(q, r, -q-r)";
        },
    ...
    methods: {
        qDoubledToOddQOffset(doubled) {
            return OffsetCoord.qoffsetFromQdoubled(OffsetCoord.ODD, doubled);
        },
        rDoubledToOddROffset(doubled) {
            return OffsetCoord.roffsetFromRdoubled(OffsetCoord.ODD, doubled);
        },
        setHighlightFlat(hex, fractionalHex) {
            this.highlightOrientation = 'flat';
            this.setHighlight(hex, fractionalHex);
        },
        setHighlightPointy(hex, fractionalHex) {
            this.highlightOrientation = 'pointy';
            this.setHighlight(hex, fractionalHex);
        },
        className(hex, orientation) {
            return {
                origin: hex.len() === 0,
                highlight: hex_equal(hex, this.highlight) && orientation === this.highlightOrientation,
            };
        },
    },
});
