...
var _CanvasHelper = class _CanvasHelper {
  /** Added by an LLM agent (P1): single grid truth - the configured setting. */
  static resolveGridSize(plugin) {
    var _a, _b;
    try {
      const value = (_b = (_a = plugin == null ? void 0 : plugin.settings) == null ? void 0 : _a.getSetting) == null ? void 0 : _b.call(_a, "gridSize");
      if (typeof value === "number" && Number.isFinite(value) && value > 0) return value;
    } catch (e) {
    }
    return _CanvasHelper.GRID_SIZE;
  }
  ...
  /** Added by an LLM agent (P1): align with the plugin's configured gridSize. */
  static alignToPluginGrid(plugin, value) {
    return _CanvasHelper.alignToGrid(value, _CanvasHelper.resolveGridSize(plugin));
  }
...
/** Added by an LLM agent (P1): legacy fallback. Use resolveGridSize(plugin) -
 * the single grid truth is the `gridSize` setting. */
...
var CanvasExtension = class {
  constructor(plugin) {
    ...
    this.init();
    try {
      this.settingsChangedRef = this.plugin.app.workspace.on(
        "advanced-canvas:settings-changed",
        () => this.syncEnabledState()
      );
      this.plugin.registerEvent(this.settingsChangedRef);
    } catch (e) {
    }
  }
  /** Override to release listeners/observers/timers created in init(). */
  dispose() {
  }
  /** Override to refresh already-initialized state when settings change. */
  onSettingsEnabled() {
  }
  isActive() {
    return this.enabled;
  ...
  settingKey() {
    ...
    return flag === true ? null : flag;
  }
  checkEnabled() {
    const key = this.settingKey();
    if (key === null) return true;
    try {
      return !!this.plugin.settings.getSetting(key);
    } catch (e) {
      return false;
    }
  }
  syncEnabledState() {
    const shouldBeEnabled = this.checkEnabled();
    if (shouldBeEnabled === this.enabled) {
      if (shouldBeEnabled) {
        try {
          this.onSettingsEnabled();
        } catch (e) {
          console.error(`Advanced Canvas: onSettingsEnabled failed for ${this.constructor.name}:`, e);
        }
      }
      return;
    }
    ...
    if (shouldBeEnabled) {
      try {
        ...
        console.error(`Advanced Canvas: late init failed for ${this.constructor.name}:`, e);
      }
      try {
        this.onSettingsEnabled();
      } catch (e) {
      ...
      try {
        ...
        console.error(`Advanced Canvas: dispose failed for ${this.constructor.name}:`, e);
      }
    }
  }
};
...
var MAX_MS_PER_EDGE = 100;
var sharedBudgetResetFrame = 0;
var sharedBudgetSpentMs = 0;
...
var ObstacleGrid = class {
  constructor(obstacles, cellSize) {
    this.cells = /* @__PURE__ */ new Map();
    this.cellSize = Math.max(cellSize, 1);
    for (const obstacle of obstacles) {
      const minCellX = Math.floor(obstacle.minX / this.cellSize);
      const maxCellX = Math.floor(obstacle.maxX / this.cellSize);
      const minCellY = Math.floor(obstacle.minY / this.cellSize);
      const maxCellY = Math.floor(obstacle.maxY / this.cellSize);
  ...
  candidates(x, y) {
    var _a;
    return (_a = this.cells.get(`${Math.floor(x / this.cellSize)},${Math.floor(y / this.cellSize)}`)) != null ? _a : [];
  }
};
var EdgePathfindingAStar = class extends EdgePathfindingMethod {
  getPath() {
    ...
    const gridSize = (_a = this.plugin.settings.getSetting("gridSize")) != null ? _a : CanvasHelper.GRID_SIZE;
    const pathArray = this.aStarAlgorithm(fromPosWithMargin, toPosWithMargin, nodeBBoxes, gridSize / 2, allowDiagonal);
  ...
  claimBudget() {
    ...
    if (frame !== sharedBudgetResetFrame) {
      sharedBudgetResetFrame = frame;
      sharedBudgetSpentMs = 0;
    }
    return Math.max(0, MAX_MS_PER_EDGE - sharedBudgetSpentMs);
  }
  aStarAlgorithm(fromPos, toPos, obstacles, gridResolution, allowDiagonal) {
    ...
    const obstacleGrid = new ObstacleGrid(obstacles, gridResolution * 8);
    if (this.isInsideObstacle(start, obstacleGrid) || this.isInsideObstacle(end, obstacleGrid)) return null;
    const openHeap = new MinHeap();
    const openByKey = /* @__PURE__ */ new Map();
    const closedSet = /* @__PURE__ */ new Set();
    start.hCost = this.heuristic(start, end, allowDiagonal);
    start.fCost = start.hCost;
    openHeap.push(start);
    openByKey.set(start.key(), start);
    const budgetMs = this.claimBudget();
    ...
    while (openHeap.size > 0) {
      ...
      if ((expansions & 63) === 0 && performance.now() - startTimestamp > budgetMs) {
        sharedBudgetSpentMs += performance.now() - startTimestamp;
        return null;
      }
      if (!(current.x === start.x && current.y === start.y) && this.isTouchingObstacle(current, obstacleGrid))
        continue;
      for (const direction of availableDirections) {
        const nx = current.x + direction.dx * gridResolution;
        const ny = current.y + direction.dy * gridResolution;
        const key = `${nx},${ny}`;
        if (closedSet.has(key)) continue;
        if (this.isInsideObstacleXY(nx, ny, obstacleGrid)) continue;
        const stepCost = direction.dx !== 0 && direction.dy !== 0 ? DIAGONAL_COST : 1;
        const tentativeGCost = current.gCost + stepCost;
        const existing = openByKey.get(key);
        if (existing) {
          if (tentativeGCost >= existing.gCost) continue;
          existing.parent = current;
          existing.gCost = tentativeGCost;
          existing.fCost = tentativeGCost + existing.hCost;
          openHeap.update(existing);
        } else {
          const neighbor = new AStarNode(nx, ny);
          neighbor.parent = current;
          neighbor.gCost = tentativeGCost;
          neighbor.hCost = this.heuristic(neighbor, end, allowDiagonal);
          neighbor.fCost = neighbor.gCost + neighbor.hCost;
          openHeap.push(neighbor);
          openByKey.set(key, neighbor);
        }
      }
    }
    return null;
  }
  // Added by an LLM agent (P1-9): octile distance when diagonal moves are
  // allowed (admissible for 8-directional grids); Manhattan otherwise. The old
  // Manhattan-with-diagonal-costs heuristic was inadmissible and over-expanded.
  heuristic(node, end, allowDiagonal) {
  ...
  isTouchingObstacle(node, obstacles) {
    return obstacles.candidates(node.x, node.y).some((obstacle) => BBoxHelper.insideBBox(node, obstacle, true));
  }
  isInsideObstacle(node, obstacles) {
    return obstacles.candidates(node.x, node.y).some((obstacle) => BBoxHelper.insideBBox(node, obstacle, false));
  }
  isInsideObstacleXY(x, y, obstacles) {
    const pos = { x, y };
    return obstacles.candidates(x, y).some((obstacle) => BBoxHelper.insideBBox(pos, obstacle, false));
  }
...
var EdgePathfindingSquare = class extends EdgePathfindingMethod {
    ...
    if (this.fromSide === this.toSide) {
      ...
      if (!isPathCollidingAtFrom || !isPathCollidingAtTo) {
        ...
        const firstFromDetourPoint = BBoxHelper.isHorizontal(this.fromSide) ? {
          x: CanvasHelper.alignToPluginGrid(this.plugin, this.fromBBoxSidePos.x + fromDirection * CanvasHelper.resolveGridSize(this.plugin)),
          y: this.fromBBoxSidePos.y
        } : {
          x: this.fromBBoxSidePos.x,
          y: CanvasHelper.alignToPluginGrid(this.plugin, this.fromBBoxSidePos.y + fromDirection * CanvasHelper.resolveGridSize(this.plugin))
        };
        const toDirection = BBoxHelper.direction(this.toSide);
        const firstToDetourPoint = BBoxHelper.isHorizontal(this.toSide) ? {
          x: CanvasHelper.alignToPluginGrid(this.plugin, this.toBBoxSidePos.x + toDirection * CanvasHelper.resolveGridSize(this.plugin)),
          y: this.toBBoxSidePos.y
        } : {
          x: this.toBBoxSidePos.x,
          y: CanvasHelper.alignToPluginGrid(this.plugin, this.toBBoxSidePos.y + toDirection * CanvasHelper.resolveGridSize(this.plugin))
        };
        ...
        if (isPathCollidingAtFrom && isPathCollidingAtTo) {
          ...
          if (BBoxHelper.isHorizontal(this.fromSide)) {
            ...
            firstFromDetourPoint = {
              x: CanvasHelper.alignToPluginGrid(this.plugin, (direction > 0 ? combinedBBoxes.maxX : combinedBBoxes.minX) + direction * CanvasHelper.resolveGridSize(this.plugin)),
              y: this.fromBBoxSidePos.y
            };
            ...
            firstFromDetourPoint = {
              x: this.fromBBoxSidePos.x,
              y: CanvasHelper.alignToPluginGrid(this.plugin, (direction > 0 ? combinedBBoxes.maxY : combinedBBoxes.minY) + direction * CanvasHelper.resolveGridSize(this.plugin))
            };
            secondFromDetourPoint = {
          ...
          if (isPathCollidingAtFrom) {
            const direction = BBoxHelper.direction(this.fromSide);
            const firstFromDetourPoint = BBoxHelper.isHorizontal(this.fromSide) ? {
              x: CanvasHelper.alignToPluginGrid(this.plugin, this.fromBBoxSidePos.x + direction * CanvasHelper.resolveGridSize(this.plugin)),
              y: this.fromBBoxSidePos.y
            } : {
              x: this.fromBBoxSidePos.x,
              y: CanvasHelper.alignToPluginGrid(this.plugin, this.fromBBoxSidePos.y + direction * CanvasHelper.resolveGridSize(this.plugin))
            };
          ...
          if (isPathCollidingAtTo) {
            ...
            const firstToDetourPoint = BBoxHelper.isHorizontal(this.toSide) ? {
              x: CanvasHelper.alignToPluginGrid(this.plugin, this.toBBoxSidePos.x + direction * CanvasHelper.resolveGridSize(this.plugin)),
              y: this.toBBoxSidePos.y
            } : {
              x: this.toBBoxSidePos.x,
              y: CanvasHelper.alignToPluginGrid(this.plugin, this.toBBoxSidePos.y + direction * CanvasHelper.resolveGridSize(this.plugin))
            };
  ...
  getUPath(fromPos, toPos, fromSide, toSide) {
    ...
    if (BBoxHelper.isHorizontal(fromSide)) {
      ...
      const x = CanvasHelper.alignToPluginGrid(this.plugin, xExtremum + direction * CanvasHelper.resolveGridSize(this.plugin));
      return {
        pathArray: [
          fromPos,
          { x, y: fromPos.y },
      ...
      const y = CanvasHelper.alignToPluginGrid(this.plugin, yExtremum + direction * CanvasHelper.resolveGridSize(this.plugin));
      return {
        pathArray: [
          fromPos,
          { x: fromPos.x, y },
...
var BUILTIN_NODE_STYLE_ATTRIBUTES = [
  ...
  },
  {
    ...
    options: [
      {
        icon: "rotate-ccw",
        label: 'Default (follows "Card node padding" setting)',
        value: null
      },
      {
        icon: "ban",
...
var BUILTIN_EDGE_STYLE_ATTRIBUTES = [
  ...
  {
    // Added by an LLM agent: consumed by the bezier curvature feature (bezier-curvature-canvas-extension.ts)
    // Only affects edges without a pathfinding method; the value scales the bezier control points
    key: "bezierCurvature",
    label: "Bezier Curvature",
    options: [
      {
        icon: "spline",
        label: "Default (100%)",
  ...
  {
    // Added by an LLM agent: consumed by the z-ordering feature (z-ordering-canvas-extension.ts)
    key: "zIndex",
    label: "Z-Index",
    options: [
      {
...
// src/utils/modal-helper.ts
var AbstractSelectionModal = class extends import_obsidian4.FuzzySuggestModal {
...
var PortalsCanvasExtension = class _PortalsCanvasExtension extends CanvasExtension {
  constructor() {
    super(...arguments);
    // Added by an LLM agent (P1-8/P1-11): debounced vault.modify handling plus a
    // per-canvas index of portal source paths so a save doesn't O(canvases x nodes)
    // scan synchronously on every keystroke-save.
    this.modifyDebounce = 0;
    this.pendingModifiedPaths = /* @__PURE__ */ new Set();
    this.portalIndex = /* @__PURE__ */ new WeakMap();
  }
  isEnabled() {
  ...
  init() {
    this.plugin.registerEvent(this.plugin.app.vault.on("modify", (file) => {
      this.pendingModifiedPaths.add(file.path);
      window.clearTimeout(this.modifyDebounce);
      this.modifyDebounce = window.setTimeout(() => this.flushModifiedFiles(), 300);
    }));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:popup-menu-created",
    ...
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      ...
      (canvas, data, setData) => {
        this.onSetData(canvas, data).then((newData) => {
          ...
          new import_obsidian5.Notice("An error occurred while loading portal data. Please check console for details.");
        });
      }
    ));
  }
  onFileModified(canvas, file) {
    if (!this.isPortalSource(canvas, file.path)) return;
    canvas.setData(canvas.getData());
    canvas.history.current--;
    canvas.history.data.pop();
  }
  /** Added by an LLM agent (P1-8/P1-11): flush debounced vault.modify events. */
  flushModifiedFiles() {
    const paths = [...this.pendingModifiedPaths];
    this.pendingModifiedPaths.clear();
    if (paths.length === 0) return;
    for (const canvas of this.plugin.getCanvases()) {
      try {
        for (const path of paths) {
          if (this.isPortalSource(canvas, path)) {
            canvas.setData(canvas.getData());
            canvas.history.current--;
            canvas.history.data.pop();
            break;
        ...
        console.error("Advanced Canvas: portal refresh failed:", e);
      }
    }
  }
  /** Added by an LLM agent (P1-8): indexed portal-source check, no full scan. */
  isPortalSource(canvas, path) {
    let index = this.portalIndex.get(canvas);
    if (!index) {
      index = this.buildPortalIndex(canvas);
      this.portalIndex.set(canvas, index);
    }
    if (index.has(path)) return true;
    const rebuilt = this.buildPortalIndex(canvas);
    this.portalIndex.set(canvas, rebuilt);
    return rebuilt.has(path);
  }
  /** Added by an LLM agent (P1-8): Map<portalSourcePath, nodeIds>. */
  buildPortalIndex(canvas) {
    var _a;
    ...
    try {
      for (const node of canvas.nodes.values()) {
        ...
        if ((data == null ? void 0 : data.type) !== "file") continue;
        const source = (_a = node.currentPortalFile) != null ? _a : data.file;
        if (!source) continue;
  ...
  onEdgeConnectionTryDraggingBefore(_canvas, edge, _event, cancelRef) {
    ...
    new import_obsidian5.Notice("Updating edges from portals is not supported yet.");
  }
  onEdgeConnectionDraggingAfter(canvas, edge, _event, _newEdge, _side, _previousEnds) {
    ...
    canvas.removeEdge(edge);
    new import_obsidian5.Notice("Creating edges with both ends in portals are not supported yet.");
  }
  onPopupMenu(canvas) {
  ...
  async tryAutoOpenSingleNodePortal(canvas, node) {
    ...
    if (!(portalFile instanceof import_obsidian5.TFile) || portalFile.extension !== "canvas") return;
  ...
  onGetData(_canvas, data) {
    ...
    data.edges = data.edges.filter((edgeData) => {
      ...
      const fromNested = _PortalsCanvasExtension.getNestedIds(edgeData.fromNode);
      const toNested = _PortalsCanvasExtension.getNestedIds(edgeData.toNode);
      const isFromNodeFromPortal = fromNested.length > 1;
      const isToNodeFromPortal = toNested.length > 1;
      if (!isFromNodeFromPortal && !isToNodeFromPortal) return true;
      if (isFromNodeFromPortal && isToNodeFromPortal) return false;
      const nestedIds = isFromNodeFromPortal ? fromNested : toNested;
      const targetPortalId = nestedIds.length < 2 ? void 0 : nestedIds.slice(0, -1).join(PORTAL_ID_DELIMITER);
      const targetPortalData = targetPortalId ? portalsIdMap.get(targetPortalId) : void 0;
  ...
  async onSetData(canvas, dataRef) {
    ...
    let data;
    try {
      data = typeof structuredClone === "function" ? structuredClone(dataRef) : JSON.parse(JSON.stringify(dataRef));
    } catch (e) {
      console.error("Advanced Canvas: portal data copy failed, using reference:", e);
      data = dataRef;
    }
  ...
  async tryOpenPortal(canvas, portalNodeData, nestedPortalFiles = /* @__PURE__ */ new Set()) {
    ...
    if (!(portalFile instanceof import_obsidian5.TFile) || portalFile.extension !== "canvas") return addedData;
    const portalFileDataString = await this.plugin.app.vault.cachedRead(portalFile);
    ...
    try {
      ...
      console.error(`Advanced Canvas: portal file is not valid JSON: ${portalNodeData.file}`, e);
      return addedData;
    }
...
var CONNECTION_FILTERS = [
  ...
  // Added by an LLM agent
  // Added by an LLM agent - one-hop directional variants without the group special cases
  { id: "filter-immediately-connected-incoming-nodes", name: "Filter to selection and immediately connected incoming nodes", outgoing: false, incoming: true, immediate: true, includeGroups: false },
  { id: "filter-immediately-connected-outgoing-nodes", name: "Filter to selection and immediately connected outgoing nodes", outgoing: true, incoming: false, immediate: true, includeGroups: false },
...
var NODE_BASED_FILTERS = [
  { id: "filter-by-color", name: "Filter by color of selection" },
  ...CONNECTION_FILTERS
];
var CanvasFilterCanvasExtension = class _CanvasFilterCanvasExtension extends CanvasExtension {
  ...
  init() {
    this.plugin.addCommand({
      ...
      checkCallback: CanvasHelper.canvasCommand(
        this.plugin,
        (_canvas) => true,
        (canvas) => this.resetFilter(canvas)
      )
    });
    this.plugin.addCommand({
      id: "filter-by-color",
  ...
  // Added by an LLM agent
  /** Shows all nodes and edges again (same as the "Reset filter" command). */
  resetFilter(canvas) {
  ...
  // Added by an LLM agent
  /**
   * Applies one of the NODE_BASED_FILTERS using the given nodes as the implicit selection
   * (used by the presentation slide filter feature). Returns false for unknown filter ids.
   */
  applyNodeBasedFilter(canvas, filterId, nodes) {
    if (filterId === "filter-by-color") {
      this.filterByColor(canvas, nodes);
      return true;
    }
    const connectionFilter = CONNECTION_FILTERS.find((filter) => filter.id === filterId);
    if (!connectionFilter) return false;
    this.filterByConnections(canvas, connectionFilter, nodes);
    return true;
  }
  // Visibility helpers
  ...
  showOnlyNodes(canvas, nodeIdsToShow) {
    if (nodeIdsToShow === void 0) canvas.wrapperEl.removeAttribute("data-ac-filter-active");
    else canvas.wrapperEl.setAttribute("data-ac-filter-active", "nodes");
    for (const node of canvas.nodes.values())
  ...
  // Filters
  // Added by an LLM agent - `elements` overrides the selection as the filter source (presentation slide filter)
  filterByColor(canvas, elements) {
    const colorsToShow = new Set((elements != null ? elements : [...canvas.selection]).map((element) => {
      var _a;
      return (_a = element.getData().color) != null ? _a : "";
    }));
    if (colorsToShow.has(""))
      new import_obsidian6.Notice("One of the selected elements has no color, so colorless nodes stay visible as well");
    const nodes = [...canvas.nodes.values()].map((node) => node.getData());
  ...
  async filterByTag(canvas) {
    ...
    if (tags.length === 0) {
      new import_obsidian6.Notice("No tags found in this vault");
      return;
    }
  ...
  // Added by an LLM agent - `startElements` overrides the selection as the traversal start (presentation slide filter)
  filterByConnections(canvas, connectionFilter, startElements) {
    var _a, _b;
    const nodes = [...canvas.nodes.values()].map((node) => node.getData());
    const edges = [...canvas.edges.values()].map((edge) => edge.getData());
    const nodeIdsToShow = new Set((startElements != null ? startElements : [...canvas.selection]).map((element) => element.id));
    ...
    for (const edge of edges) {
      if (connectionFilter.immediate && edge.fromEnd === "arrow") continue;
      let list = outgoing.get(edge.fromNode);
      if (!list) {
        list = [];
        outgoing.set(edge.fromNode, list);
      }
      list.push(edge);
      list = incoming.get(edge.toNode);
      if (!list) {
        list = [];
        incoming.set(edge.toNode, list);
      }
      list.push(edge);
    }
    const groupNodes = nodes.filter((node) => node.type === "group");
    const portalNodes = nodes.filter((node) => node.type === "file" && node.portal);
    let frontier = new Set(nodeIdsToShow);
    while (frontier.size > 0) {
      const nextFrontier = /* @__PURE__ */ new Set();
      for (const nodeId of frontier) {
        if (connectionFilter.outgoing) {
          for (const edge of (_a = outgoing.get(nodeId)) != null ? _a : []) {
            edgeIdsToShow.add(edge.id);
            if (nodeIdsToShow.has(edge.toNode)) continue;
            nodeIdsToShow.add(edge.toNode);
            nextFrontier.add(edge.toNode);
          }
        }
        if (connectionFilter.incoming) {
          for (const edge of (_b = incoming.get(nodeId)) != null ? _b : []) {
            edgeIdsToShow.add(edge.id);
            if (nodeIdsToShow.has(edge.fromNode)) continue;
            nodeIdsToShow.add(edge.fromNode);
      ...
      if (connectionFilter.includeGroups !== false) {
  ...
  getTagsOfNode(canvas, nodeData) {
    ...
    if (nodeData.type === "file") {
      ...
      return (_b = (0, import_obsidian6.getAllTags)(fileMetadata)) != null ? _b : [];
    }
    if (nodeData.type === "text") {
      ...
      if (nodeMetadata) return (_d = (0, import_obsidian6.getAllTags)(nodeMetadata)) != null ? _d : [];
...
var DEFAULT_SETTINGS_VALUES = {
  ...
  // Added by an LLM agent
  nodePreviewTextSize: "1",
  // Added by an LLM agent
  nodeOutlineBlack: false,
  // Added by an LLM agent
  nodeOutlineWhite: false,
  // Added by an LLM agent
  nodeOutlineWidth: 2,
  // Added by an LLM agent
  edgeOutlineBlack: false,
  // Added by an LLM agent
  edgeOutlineWhite: false,
  // Added by an LLM agent
  edgeOutlineWidth: 2,
  // Added by an LLM agent
  ...
  // Added by an LLM agent
  hideNodeContentsFeatureEnabled: true,
  // Added by an LLM agent
  textWrapAroundFeatureEnabled: true,
  // Added by an LLM agent
...
var SETTINGS = {
  general: {
    ...
    children: {
      ...
      hideNoteLabels: {
        // Added by an LLM agent
        label: "Hide note labels",
        description: 'When enabled, labels of file/text/link nodes are hidden. Can be toggled with the "Toggle note label visibility" command.',
        type: "boolean"
      },
      boxedNodeLabels: {
        // Added by an LLM agent
      ...
      nodePreviewTextSize: {
        // Added by an LLM agent
        label: "Node preview text size",
        description: "The size of the note title shown in place of a node's content when zoomed out. 1x is Obsidian's default.",
        type: "dropdown",
        options: {
          "1": "Default (1x)",
          "1.5": "Large (1.5x)",
      ...
      nodeOutlineBlack: {
        // Added by an LLM agent
        label: "Black node outline",
        description: 'When enabled, nodes get a black outline around their border. If "White node outline" is also enabled, the black outline becomes the inner ring and the white outline the outer ring.',
        type: "boolean"
      },
      nodeOutlineWhite: {
        // Added by an LLM agent
        label: "White node outline",
        description: 'When enabled, nodes get a white outline around their border. If "Black node outline" is also enabled, the white outline becomes the outer ring.',
        type: "boolean"
      },
      nodeOutlineWidth: {
        // Added by an LLM agent
        label: "Node outline width",
        description: "The width of each node outline ring, in pixels.",
        type: "number",
        parse: (value) => Math.max(1, parseInt(value) || 1)
      },
      edgeOutlineBlack: {
        // Added by an LLM agent
        label: "Black edge outline",
        description: 'When enabled, edges get a black outline around their line. If "White edge outline" is also enabled, the black outline becomes the inner ring and the white outline the outer ring.',
        type: "boolean"
      },
      edgeOutlineWhite: {
        // Added by an LLM agent
        label: "White edge outline",
        description: 'When enabled, edges get a white outline around their line. If "Black edge outline" is also enabled, the white outline becomes the outer ring.',
        type: "boolean"
      },
      edgeOutlineWidth: {
        // Added by an LLM agent
        label: "Edge outline width",
        description: "The width of each edge outline ring, in pixels. Adds onto the edge's own width, including the edge width style.",
        type: "number",
        parse: (value) => Math.max(1, parseInt(value) || 1)
      }
    }
  ...
  presentationFeatureEnabled: {
    ...
    children: {
      ...
      // Added by an LLM agent
      presentationSlideFilter: {
        label: "Default slide filter",
        description: `When a presentation navigates to a slide that has a "bidirectional" edge (an edge to the slide with an arrow on the incoming node's end) from exactly one node, this filter gets applied using that node as the selection. Can be overridden per node via the node context menu ("Slide filter: \u2026"), which stores the choice as the node's slideFilter property in the canvas file. Requires the canvas filter feature.`,
        type: "dropdown",
        options: {
          none: "None",
          ...Object.fromEntries(NODE_BASED_FILTERS.map((filter) => [filter.id, filter.name]))
        }
      }
    }
  },
  zOrderingControlFeatureEnabled: {
    ...
    // Added by an LLM agent: description updated to cover edges and the group node layering
    description: "Change the persistent z-index of nodes and edges using the context menu. Default layering: group nodes below edges, edges below other nodes.",
    children: {
      zOrderingControlShowOneLayerShiftOptions: {
        label: "Show one layer shift options",
        description: "When enabled, you can move nodes and edges one layer forward or backward.",
        type: "boolean"
      }
    }
  },
  ...
  hideNodeContentsFeatureEnabled: {
    // Added by an LLM agent
    label: "Hide node contents",
    description: `Adds a "Hide node contents" option to the node context menu (right click) that hides everything inside the node's borders, leaving an empty node frame. Not available for group nodes.`,
    children: {}
  },
  textWrapAroundFeatureEnabled: {
    // Added by an LLM agent
    label: "Text wrap-around",
    description: 'Adds a "Wrap text around overlapping nodes" option to the node context menu (right click) and the node popup menu. When enabled on a note or card node, its text reflows around nodes stacked on top of it instead of being hidden behind them. Only nodes connected to it by a bidirectional edge pointing into it count as obstacles (keeps large canvases fast). Not available for group and link nodes.',
    children: {}
  },
...
var SettingsManager = class {
  constructor(plugin) {
    // Added by an LLM agent (P0/P1-11): debounced disk writes - text/number fields
    // used to saveData + fan out settings-changed per keystroke.
    this.saveDebounce = 0;
  ...
  async loadSettings() {
    var _a;
    const saved = (_a = await this.plugin.loadData()) != null ? _a : {};
    const merged = { ...DEFAULT_SETTINGS_VALUES };
    for (const [key, value] of Object.entries(saved)) {
      if (!(key in DEFAULT_SETTINGS_VALUES)) {
        console.warn(`Advanced Canvas: ignoring unknown saved setting "${key}" (removed or renamed).`);
        continue;
      }
      const def = DEFAULT_SETTINGS_VALUES[key];
      if (Array.isArray(def) && Array.isArray(value)) merged[key] = [...value];
      else if (def !== null && typeof def === "object" && !Array.isArray(def) && value !== null && typeof value === "object" && !Array.isArray(value))
        merged[key] = { ...def, ...value };
      else merged[key] = value;
    }
    this.settings = merged;
    this.plugin.app.workspace.trigger("advanced-canvas:settings-changed", Object.keys(merged));
  }
  async saveSettings() {
  ...
  async setSetting(data, options = {}) {
    var _a;
    const keys = Object.keys(data);
    for (const key of keys) {
      if (!(key in DEFAULT_SETTINGS_VALUES)) {
        console.warn(`Advanced Canvas: unknown setting "${key}" ignored. Did you mean one of: ${Object.keys(DEFAULT_SETTINGS_VALUES).slice(0, 8).join(", ")}...?`);
        delete data[key];
      }
    }
    ...
    for (const key of Object.keys(data)) {
      if (!Object.is(this.settings[key], data[key])) {
        changed = true;
        break;
      }
    }
    this.settings = Object.assign(this.settings, data);
    for (const key of Object.keys(data)) this.pendingKeys.add(key);
    const debounceMs = (_a = options.debounceMs) != null ? _a : 0;
    if (debounceMs > 0) {
      window.clearTimeout(this.saveDebounce);
      this.saveDebounce = window.setTimeout(() => void this.flushSettings(), debounceMs);
      if (changed) this.plugin.app.workspace.trigger("advanced-canvas:settings-changed", [...this.pendingKeys]);
      return;
    }
    await this.flushSettings(changed);
  }
  /** Added by an LLM agent (P0/P1-11): write pending settings + fire one event. */
  async flushSettings(fireEvent = true) {
    window.clearTimeout(this.saveDebounce);
    const keys = [...this.pendingKeys];
    this.pendingKeys.clear();
    await this.saveSettings();
    if (fireEvent) this.plugin.app.workspace.trigger("advanced-canvas:settings-changed", keys);
  }
  addSettingsTab() {
...
var AdvancedCanvasPluginSettingTab = class extends import_obsidian7.PluginSettingTab {
  ...
  createFeatureHeading(containerEl, label, description, infoSection, settingsKey) {
    const setting = new import_obsidian7.Setting(containerEl).setHeading().setClass("ac-settings-heading").setName(label).setDesc(description);
    ...
    if (settingsKey !== null) {
      setting.addToggle(
        (toggle) => toggle.setTooltip("Takes effect immediately.").setValue(this.settingsManager.getSetting(settingsKey)).onChange(async (value) => {
  ...
  createTextSetting(containerEl, settingId, setting) {
    new import_obsidian7.Setting(containerEl).setName(setting.label).setDesc(setting.description).addText(
      (text) => text.setValue(this.settingsManager.getSetting(settingId)).onChange(async (value) => {
        await this.settingsManager.setSetting({ [settingId]: setting.parse ? setting.parse(value) : value }, { debounceMs: 400 });
      })
    );
  }
  createNumberSetting(containerEl, settingId, setting) {
    new import_obsidian7.Setting(containerEl).setName(setting.label).setDesc(setting.description).addText(
      (text) => text.setValue(JSON.stringify(this.settingsManager.getSetting(settingId))).onChange(async (value) => {
        await this.settingsManager.setSetting({ [settingId]: setting.parse(value) }, { debounceMs: 400 });
      })
    );
  }
  createDimensionSetting(containerEl, settingId, setting) {
    let text1;
    let text2;
    new import_obsidian7.Setting(containerEl).setName(setting.label).setDesc(setting.description).addText((text) => {
  ...
  createBooleanSetting(containerEl, settingId, setting) {
    new import_obsidian7.Setting(containerEl).setName(setting.label).setDesc(setting.description).addToggle(
  ...
  createDropdownSetting(containerEl, settingId, setting) {
    new import_obsidian7.Setting(containerEl).setName(setting.label).setDesc(setting.description).addDropdown(
  ...
  createButtonSetting(containerEl, settingId, setting) {
    new import_obsidian7.Setting(containerEl).setName(setting.label).setDesc(setting.description).addButton(
  ...
  createStylesSetting(containerEl, settingId, setting) {
    ...
    for (const styleAttribute of setting.getParameters(this.settingsManager)) {
      new import_obsidian7.Setting(nestedContainerEl).setName(styleAttribute.label).addDropdown(
        (dropdown) => {
          var _a;
...
var Patcher = class _Patcher {
  ...
  static async waitForMapValueLookup(map, viewType, patch) {
    return new Promise((resolve) => {
      ...
      const uninstaller = around(map, {
        // Added by an LLM agent (P0-1): call next() exactly once and patch the
        // returned view. The previous code constructed the view, patched it,
        // threw it away, then constructed + returned a second unpatched view.
        [viewType]: (next) => function(...args) {
          ...
          if (!settled) {
            ...
            try {
              ...
              console.error(`Advanced Canvas: failed to patch view "${viewType}":`, e);
            }
            uninstaller();
            resolve(view);
          }
  ...
  static patchThisAndPrototype(plugin, object, patches, uninstallers) {
    const prototyped = _Patcher.patchPrototype(plugin, object, patches, uninstallers);
    if (prototyped) return prototyped;
    return _Patcher.patch(plugin, object, patches, false, uninstallers);
  }
  ...
  static tryPatchWorkspacePrototype(plugin, getTarget, patches, uninstallers) {
    return new Promise((resolve) => {
      ...
      const listener = plugin.app.workspace.on("layout-change", () => {
        ...
        let attempt = null;
        try {
          attempt = _Patcher.patchPrototype(plugin, getTarget(), patches, uninstallers);
        } catch (e) {
          console.error("Advanced Canvas: workspace prototype patch attempt failed:", e);
          return;
        }
        if (attempt) {
          settled = true;
...
// src/patchers/canvas-patcher.ts
var _CanvasPatcher = class _CanvasPatcher extends Patcher {
  constructor() {
    super(...arguments);
    // Added by an LLM agent (P0-7/P1-10): per-element uninstallers so removeNode /
    // removeEdge can release the ~12 per-node wrappers instead of leaking them
    // in the global plugin.register list for the session.
    this.elementUninstallers = /* @__PURE__ */ new WeakMap();
    // Added by an LLM agent (P0): re-entrancy guard for the updateSelection writer.
    this.inSelectionUpdate = false;
    // Added by an LLM agent (P0-7): coalesced canvas saves - many setData calls
    // per gesture collapse into one requestSave per frame instead of one per node.
    this.pendingSaves = /* @__PURE__ */ new WeakSet();
  }
  async patch() {
    const loadedCanvasViewLeafs = this.plugin.app.workspace.getLeavesOfType("canvas").filter((leaf) => !(0, import_obsidian8.requireApiVersion)("1.7.2") || !leaf.isDeferred);
  ...
  patchCanvas(view) {
    ...
    Patcher.patchPrototype(this.plugin, view.canvas, {
      ...
      zoomToRealBbox: (_next) => function(bbox) {
        ...
        const zoom = this.screenshotting ? Math.min(widthZoom, heightZoom) : Math.clamp(Math.min(widthZoom, heightZoom), -4, CanvasHelper.MAX_ALLOWED_ZOOM);
      ...
      undo: Patcher.OverrideExisting((next) => function(...args) {
        const result = next.call(this, ...args);
        this.importData(this.getData(), true, true);
    ...
    this.plugin.registerEditorExtension([import_view.EditorView.updateListener.of((update) => {
      ...
      if (that.plugin.getCanvases().length === 0) return;
      let editor;
      try {
        editor = update.state.field(import_obsidian8.editorInfoField);
      } catch (e) {
        return;
      }
      const node = editor.node;
      if (!(node == null ? void 0 : node.canvas)) return;
  ...
  patchNode(node) {
    ...
    Patcher.patch(this.plugin, node, {
      ...
      setData: Patcher.OverrideExisting((next) => function(data, addHistory) {
        ...
        if (this.initialized && !((_a = this.canvas) == null ? void 0 : _a.isClearing)) {
          const canvasData = this.canvas.getData();
          this.canvas.data = canvasData;
          that.requestCoalescedSave(this.canvas);
          if (addHistory) this.canvas.pushHistory(canvasData);
        }
        return result;
      }),
      ...
      renderZIndex: Patcher.OverrideExisting((next) => function() {
        ...
        if (persistentZIndex === void 0) {
          ...
          that.applyGroupZIndexOffset(this);
          return result;
        }
    ...
    that.elementUninstallers.set(node, uninstallers);
  ...
  // Added by an LLM agent: group nodes render below edges (the edge SVGs sit at z-index auto),
  // regular nodes stay above. Only the rendered CSS z-index is offset - the logical zIndex
  // (used by the z-ordering commands and zIndexCounter) is left untouched.
  applyGroupZIndexOffset(node) {
    if (!this.plugin.settings.getSetting("zOrderingControlFeatureEnabled")) return;
    if (node.getData().type !== "group") return;
    node.nodeEl.style.zIndex = (node.zIndex - _CanvasPatcher.GROUP_Z_INDEX_OFFSET).toString();
  }
  patchEdge(edge) {
    const that = this;
    const uninstallers = [];
    ...
    that.elementUninstallers.set(edge, uninstallers);
    this.runAfterInitialized(edge, () => {
      this.plugin.app.workspace.trigger("advanced-canvas:edge-added", edge.canvas, edge);
    });
  }
  /** Added by an LLM agent (P0-7): run + forget the per-element uninstallers. */
  uninstallElement(element) {
    ...
    for (const uninstall of uninstallers) {
      try {
        ...
        console.error("Advanced Canvas: element uninstaller failed:", e);
      }
    }
  }
  /** Added by an LLM agent (P0-7/P1-10): at most one requestSave per canvas per frame. */
  requestCoalescedSave(canvas) {
    if (!(canvas == null ? void 0 : canvas.view)) return;
    if (this.pendingSaves.has(canvas)) return;
    this.pendingSaves.add(canvas);
    const save = () => {
      this.pendingSaves.delete(canvas);
      try {
        canvas.view.requestSave();
      } catch (e) {
        console.error("Advanced Canvas: coalesced save failed:", e);
      }
    };
    if (typeof window.requestAnimationFrame === "function") window.requestAnimationFrame(save);
    else queueMicrotask(save);
  }
...
// Added by an LLM agent: rendered z-index offset that keeps group nodes below the edge SVGs
_CanvasPatcher.GROUP_Z_INDEX_OFFSET = 1e6;
var CanvasPatcher = _CanvasPatcher;
  ...
  async patch() {
    ...
    Patcher.patchThisAndPrototype(this.plugin, suggestManager, {
      getHeadingSuggestions: Patcher.OverrideExisting((next) => async function(context, path, subpath) {
        ...
        if (!(targetFile instanceof import_obsidian9.TFile) || targetFile.extension !== "canvas")
          return result;
...
var AdvancedCanvasEmbed = class extends import_obsidian10.Component {
  ...
  async loadFile() {
    ...
    await import_obsidian10.MarkdownRenderer.render(this.context.app, nodeContent, markdownContainer, this.file.path, this);
  }
};
...
var TaskQueue = class {
  ...
  clear() {
    for (let i = this.queueHead; i < this.queue.length; i++)
      this.queue[i].reject(new Error("TaskQueue cleared"));
    this.queue = [];
    this.queueHead = 0;
  }
  async run() {
    ...
    try {
      while (this.queueHead < this.queue.length) {
        ...
        try {
          ...
          console.error("Advanced Canvas: TaskQueue task failed:", e);
          entry.reject(e);
        }
      ...
      try {
        ...
        console.error("Advanced Canvas: TaskQueue onFinished failed:", e);
      }
    }
  }
};
  ...
  async patch() {
    ...
    Patcher.patchPrototype(this.plugin, this.plugin.app.metadataCache, {
      ...
      computeFileMetadataAsync: Patcher.OverrideExisting((next) => async function(file, ...args) {
        if (file instanceof import_obsidian11.TFile && (file == null ? void 0 : file.extension) === "canvas")
...
var _CanvasMetadataHandler = class _CanvasMetadataHandler {
  ...
  static async updateMetadataCache(file) {
    ...
    const slowIndexingTimeout = window.setTimeout(() => {
      new import_obsidian11.Notice(`Canvas indexing taking a long time for file ${file.path}`);
    }, 1e4);
    try {
  ...
  static async computeCanvasMetadataAsync(data) {
    ...
    metadata.links = [];
    metadata.embeds = [];
    const textNodes = ((_d = content.nodes) != null ? _d : []).map((node, index) => ({ node, index })).filter(({ node }) => node.type === "text");
    const CONCURRENCY = 8;
    const nodeCache = _CanvasMetadataHandler.nodeMetadataCache;
    for (let i = 0; i < textNodes.length; i += CONCURRENCY) {
      ...
      await Promise.all(batch.map(async ({ node, index }) => {
        ...
        const text = (_a2 = node.text) != null ? _a2 : "";
        let cacheKey = null;
        if (text.length < 2e4) {
          let hash = 0;
          for (let j = 0; j < text.length; j++) hash = hash * 31 + text.charCodeAt(j) | 0;
          cacheKey = `${text.length}:${hash}`;
          const cached = nodeCache.get(cacheKey);
          if (cached) {
            metadata.nodes[node.id] = cached.metadata;
            metadata.links.push(...cached.links.map((link) => ({ ...link, position: { nodeId: node.id, start: { line: 0, col: 1, offset: 0 }, end: { line: 0, col: 1, offset: index } } })));
            metadata.embeds.push(...cached.embeds.map((embed2) => ({ ...embed2, position: { nodeId: node.id, start: { line: 0, col: 1, offset: 0 }, end: { line: 0, col: 1, offset: index } } })));
            return;
          }
        }
        ...
        let nodeMetadata;
        try {
          nodeMetadata = await this.computeMetadataAsync(buffer);
        } catch (e) {
          console.error(`Advanced Canvas: per-node metadata parse failed (node ${node.id}):`, e);
          return;
        }
        if (!nodeMetadata) return;
        metadata.nodes[node.id] = nodeMetadata;
        const links = ((_b2 = nodeMetadata.links) != null ? _b2 : []).map((link) => ({
          ...link,
          position: {
            nodeId: node.id,
        ...
        const embeds = ((_c2 = nodeMetadata.embeds) != null ? _c2 : []).map((embed2) => ({
          ...embed2,
          position: {
            nodeId: node.id,
            start: { line: 0, col: 1, offset: 0 },
        ...
        metadata.embeds.push(...embeds);
        if (cacheKey) {
          nodeCache.set(cacheKey, { metadata: nodeMetadata, links: (_d2 = nodeMetadata.links) != null ? _d2 : [], embeds: (_e2 = nodeMetadata.embeds) != null ? _e2 : [] });
          if (nodeCache.size > 500) {
            const oldest = nodeCache.keys().next().value;
            if (oldest !== void 0) nodeCache.delete(oldest);
          }
        }
      }));
    }
...
/* eslint-disable @typescript-eslint/no-explicit-any -- cache stores raw computeMetadataAsync results */
// Added by an LLM agent (P1): per-text metadata cache (see computeCanvasMetadataAsync).
_CanvasMetadataHandler.nodeMetadataCache = /* @__PURE__ */ new Map();
    ...
    Patcher.patchPrototype(this.plugin, this.plugin.app.vault, {
      recurseChildrenAC: (_next) => function(origin, traverse) {
        for (let stack = [origin]; stack.length > 0; ) {
          ...
          if (current) {
            traverse(current);
            if (current instanceof import_obsidian12.TFolder) stack = stack.concat(current.children);
          }
        }
      },
      getMarkdownFiles: Patcher.OverrideExisting((next) => function() {
        ...
        this.recurseChildrenAC(root, (child) => {
          if (child instanceof import_obsidian12.TFile && (child.extension === "md" || child.extension === "canvas")) {
            files.push(child);
          }
        });
        return files;
      ...
      Patcher.patchPrototype(this.plugin, view.outgoingLink, {
        recomputeLinks: Patcher.OverrideExisting((next) => function(...args) {
          ...
          try {
            ...
            if (isCanvas && this.file) this.file.extension = "canvas";
          }
        }),
    ...
    Patcher.patch(this.plugin, this.plugin.app.fileManager, {
      processFrontMatter: Patcher.OverrideExisting((next) => async function(file, fn, options) {
        if ((file == null ? void 0 : file.extension) === "canvas") {
          try {
            await that.plugin.app.vault.process(file, (data) => {
              var _a, _b, _c;
              let content;
              try {
                content = JSON.parse(data);
              } catch (e) {
                console.error("Advanced Canvas: cannot update frontmatter, canvas file is not valid JSON:", file.path, e);
                return data;
              }
              (_a = content.metadata) != null ? _a : content.metadata = {};
              (_c = (_b = content.metadata).frontmatter) != null ? _c : _b.frontmatter = {};
              try {
                fn(content.metadata.frontmatter);
              } catch (e) {
                console.error("Advanced Canvas: frontmatter callback failed:", e);
              }
              return JSON.stringify(content, null, 2);
            });
          } catch (e) {
            console.error("Advanced Canvas: failed to update frontmatter in canvas file:", file.path, e);
          }
          return;
        }
              ...
              try {
                await this.app.vault.process(this.file, (data) => {
                  ...
                  try {
                    ...
                    console.error("Advanced Canvas: cannot save frontmatter, canvas file is not valid JSON:", this.file.path, e);
                    return data;
                  }
                  if (!(content == null ? void 0 : content.metadata)) {
                    console.error("Advanced Canvas: cannot save frontmatter, canvas file has no metadata section:", this.file.path);
                    return data;
                  }
                ...
                console.error("Advanced Canvas: failed to save frontmatter in canvas file:", (_a2 = this.file) == null ? void 0 : _a2.path, e);
              }
            })();
            return;
          }
...
var CanvasSearchView = class {
  constructor(view) {
    // Added by an LLM agent (P0-5): debounce handle for search-on-type.
  ...
  createSearchView() {
    ...
    this.searchInput.addEventListener("input", () => {
      window.clearTimeout(this.inputDebounce);
      this.inputDebounce = window.setTimeout(() => this.onInput(), 120);
    });
    ...
    (0, import_obsidian13.setIcon)(previousButton, "arrow-up");
    ...
    (0, import_obsidian13.setIcon)(nextButton, "arrow-down");
  ...
  onInput() {
    ...
    if (!hasQuery) this.searchMatches = [];
      ...
      let regex;
      try {
        regex = new RegExp(escapeRegExp(this.searchInput.value), "gi");
      } catch (e) {
        this.searchMatches = [];
        this.changeMatch(0);
        return;
      ...
      this.searchMatches = Array.from(this.view.canvas.nodes.values()).map((node) => {
        ...
        if (nodeData.type === "text") content = nodeData.text;
          ...
          try {
            content = (_a = node.child) == null ? void 0 : _a.data;
          } catch (e) {
            content = void 0;
          }
        }
        if (!content || !content.toLowerCase().includes(query)) return null;
        const matches = [];
        try {
          regex.lastIndex = 0;
          let match;
...
var DotMdFileSearchPatcher = class extends Patcher {
  ...
  async patch() {
    ...
    const that = this;
    Patcher.patch(this.plugin, import_obsidian14.Modal.prototype, {
  ...
  onModalOpen(modal) {
    ...
    Patcher.patch(this.plugin, modal, {
      ...
      renderSuggestion: (next) => function(item, el) {
        ...
        (0, import_obsidian14.renderMatches)(titleEl, item.acDotMdPath, (_b = (_a = item.match) == null ? void 0 : _a.matches) != null ? _b : null);
      ...
      onChooseSuggestion: (next) => function(item, evt) {
        ...
        try {
          ...
          new import_obsidian14.Notice('Advanced Canvas: Failed to add the ".md" file to the canvas.');
        }
      }
    });
  }
  getDotMdSuggestions(query) {
    ...
    const search = (0, import_obsidian14.prepareFuzzySearch)(trimmed);
    const suggestions = [];
...
var MetadataCanvasExtension = class extends CanvasExtension {
  ...
  onCanvasChanged(canvas) {
    ...
    if (!metadata || metadata.version !== CURRENT_SPEC_VERSION)
      return void new import_obsidian15.Notice("Metadata node not found or version mismatch. Should have been migrated (but wasn't).");
    const that = this;
    const validator = {
      get(target, key) {
...
var GroupLabelVisibilityCanvasExtension = class extends CanvasExtension {
  ...
  toggleGroupLabelVisibility() {
    ...
    new import_obsidian16.Notice(hidden ? "Group labels hidden" : "Group labels visible");
  }
};

// src/canvas-extensions/note-label-visibility-canvas-extension.ts
var import_obsidian17 = require("obsidian");
var NoteLabelVisibilityCanvasExtension = class extends CanvasExtension {
  isEnabled() {
    return true;
  }
  init() {
    this.plugin.addCommand({
      id: "toggle-note-label-visibility",
      name: "Toggle note label visibility",
      checkCallback: CanvasHelper.canvasCommand(
        this.plugin,
        (_canvas) => true,
        (_canvas) => this.toggleNoteLabelVisibility()
      )
    });
  }
  toggleNoteLabelVisibility() {
    const hidden = !this.plugin.settings.getSetting("hideNoteLabels");
    void this.plugin.settings.setSetting({ hideNoteLabels: hidden });
    new import_obsidian17.Notice(hidden ? "Note labels hidden" : "Note labels visible");
  }
};
...
var DEFAULT_SIZE_SETTINGS = [
  ...
  // Added by an LLM agent - not a per-element style default, but the same wrapper-attribute
  // mechanism: scales the .canvas-node-placeholder title shown when zoomed out
  { setting: "nodePreviewTextSize", datasetKey: "nodePreviewTextSize" }
];
...
// src/canvas-extensions/node-outline-canvas-extension.ts
var OUTLINE_WIDTH_PROPERTY = "--ac-node-outline-width";
var NodeOutlineCanvasExtension = class extends CanvasExtension {
  isEnabled() {
    return true;
  }
  init() {
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      ...
      () => {
        for (const canvas of this.plugin.getCanvases())
          this.updateNodeOutline(canvas);
      }
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:canvas-changed",
      (canvas) => this.updateNodeOutline(canvas)
    ));
  }
  updateNodeOutline(canvas) {
    const wrapperEl = canvas.wrapperEl;
    if (!wrapperEl) return;
    const black = this.plugin.settings.getSetting("nodeOutlineBlack");
    const white = this.plugin.settings.getSetting("nodeOutlineWhite");
    if (!black && !white) {
      delete wrapperEl.dataset.nodeOutline;
      wrapperEl.style.removeProperty(OUTLINE_WIDTH_PROPERTY);
      return;
    }
    wrapperEl.dataset.nodeOutline = black && white ? "both" : black ? "black" : "white";
    const width = this.plugin.settings.getSetting("nodeOutlineWidth");
    wrapperEl.style.setProperty(OUTLINE_WIDTH_PROPERTY, `${width}px`);
  }
};

// src/canvas-extensions/edge-outline-canvas-extension.ts
var OUTLINE_WIDTH_PROPERTY2 = "--ac-edge-outline-width";
var OUTLINE_CLASS = "ac-edge-outline-path";
var OUTER_OUTLINE_CLASS = "ac-edge-outline-outer";
var ARROW_OUTLINE_CLASS = "ac-edge-outline-arrow";
var ARROW_OUTER_OUTLINE_CLASS = "ac-edge-outline-arrow-outer";
var ARROW_SOURCE_EXPANDO = "__acArrowOutlineSource";
var ARROW_CLONES_EXPANDO = "__acArrowOutlineClones";
var EdgeOutlineCanvasExtension = class extends CanvasExtension {
  constructor() {
    super(...arguments);
    this.edgeRaf = /* @__PURE__ */ new WeakMap();
    this.pendingEdges = /* @__PURE__ */ new WeakMap();
  }
  // Added by an LLM agent (P1-10/P1-12): the old isEnabled() { return true }
  // doubled every edge's DOM even when both outline toggles were off. Gate on
  // the actual settings, and skip all work when disabled.
  isEnabled() {
    return true;
  }
  isOutlineActive() {
    return this.isActive() && this.outlineMode() !== "none";
  }
  init() {
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:settings-changed",
      () => {
        if (!this.isOutlineActive()) {
          for (const canvas of this.plugin.getCanvases()) {
            this.updateWrapper(canvas);
            for (const edge of canvas.edges.values()) this.syncEdge(edge);
          }
          return;
        }
        for (const canvas of this.plugin.getCanvases()) {
    ...
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:canvas-changed",
      (canvas) => {
        if (!this.isOutlineActive()) return;
        this.updateWrapper(canvas);
        for (const edge of canvas.edges.values()) this.syncEdge(edge);
      }
    ));
    const syncEdge = (_canvas, edge) => {
      if (!this.isOutlineActive()) return;
      this.queueEdgeSync(_canvas, edge);
    };
    this.plugin.registerEvent(this.plugin.app.workspace.on("advanced-canvas:edge-added", syncEdge));
    this.plugin.registerEvent(this.plugin.app.workspace.on("advanced-canvas:edge-changed", syncEdge));
    this.plugin.registerEvent(this.plugin.app.workspace.on("advanced-canvas:edge-path-updated", syncEdge));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:edge-removed",
      (_canvas, edge) => this.cleanupEdge(edge)
    ));
  }
  onSettingsEnabled() {
    for (const canvas of this.plugin.getCanvases()) {
  ...
  /** Added by an LLM agent (P1-10): batch edge syncs to one rAF per canvas. */
  queueEdgeSync(canvas, edge) {
    let pending = this.pendingEdges.get(canvas);
    if (!pending) {
      pending = /* @__PURE__ */ new Set();
      this.pendingEdges.set(canvas, pending);
    }
    pending.add(edge);
    if (this.edgeRaf.has(canvas)) return;
    this.edgeRaf.set(canvas, window.requestAnimationFrame(() => {
      this.edgeRaf.delete(canvas);
      const batch = this.pendingEdges.get(canvas);
      this.pendingEdges.delete(canvas);
      if (!batch) return;
      for (const e of batch) {
        try {
          this.syncEdge(e);
        } catch (err) {
          console.error("Advanced Canvas: edge outline sync failed:", err);
        }
      }
    }));
  }
  /** Added by an LLM agent (P1-10): drop expando-tracked clones of a removed edge. */
  cleanupEdge(edge) {
    try {
      const group = edge.lineEndGroupEl;
      if (group) {
        for (const clone of Array.from(group.querySelectorAll(`.${ARROW_OUTLINE_CLASS}`))) clone.remove();
      }
      const lineGroup = edge.lineGroupEl;
      if (lineGroup) {
        for (const underlay of Array.from(lineGroup.querySelectorAll(`:scope > path.${OUTLINE_CLASS}`))) underlay.remove();
      }
    } catch (e) {
    }
  }
  outlineMode() {
    const black = this.plugin.settings.getSetting("edgeOutlineBlack");
    const white = this.plugin.settings.getSetting("edgeOutlineWhite");
    if (black && white) return "both";
    if (black) return "black";
    if (white) return "white";
    return "none";
  }
  updateWrapper(canvas) {
    const wrapperEl = canvas.wrapperEl;
    if (!wrapperEl) return;
    const mode = this.outlineMode();
    if (mode === "none") {
      delete wrapperEl.dataset.edgeOutline;
      wrapperEl.style.removeProperty(OUTLINE_WIDTH_PROPERTY2);
      return;
    }
    wrapperEl.dataset.edgeOutline = mode;
    const width = this.plugin.settings.getSetting("edgeOutlineWidth");
    wrapperEl.style.setProperty(OUTLINE_WIDTH_PROPERTY2, `${width}px`);
  }
  syncEdge(edge) {
    const mode = this.outlineMode();
    const needed = mode === "both" ? 2 : mode === "none" ? 0 : 1;
    this.syncLine(edge, needed);
    this.syncArrowHeads(edge, needed);
  }
  syncLine(edge, needed) {
    var _a;
    const lineGroup = edge.lineGroupEl;
    const display = (_a = edge.path) == null ? void 0 : _a.display;
    if (!lineGroup || !display || display.parentElement !== lineGroup) return;
    const underlays = Array.from(lineGroup.querySelectorAll(`:scope > path.${OUTLINE_CLASS}`));
    while (underlays.length > needed) underlays.pop().remove();
    while (underlays.length < needed) {
      const underlay = lineGroup.ownerDocument.createElementNS("http://www.w3.org/2000/svg", "path");
      underlay.classList.add(OUTLINE_CLASS);
      lineGroup.insertBefore(underlay, display);
      underlays.push(underlay);
    }
    underlays.forEach((underlay, index) => {
      var _a2;
      underlay.classList.toggle(OUTER_OUTLINE_CLASS, needed === 2 && index === 0);
      const d = (_a2 = display.getAttribute("d")) != null ? _a2 : "";
      if (underlay.getAttribute("d") !== d) underlay.setAttribute("d", d);
      for (const key of Object.keys(underlay.dataset))
        if (!(key in display.dataset)) delete underlay.dataset[key];
      for (const [key, value] of Object.entries(display.dataset))
        if (underlay.dataset[key] !== value) underlay.dataset[key] = value;
    });
  }
  /**
   * Maintains the underlay clones of the arrow head(s) (`polygon.canvas-path-end` inside
   * `lineEndGroupEl`). Each clone keeps the `canvas-path-end` class, so app.css and the arrow
   * style rules (scale transform, arrow size multiplier) apply to it unchanged - the SCSS
   * only swaps fill/stroke/stroke-width. The clone/source link is tracked with expandos, so
   * underlays of arrow elements that Obsidian replaced get removed again.
   */
  syncArrowHeads(edge, needed) {
    var _a;
    const group = edge.lineEndGroupEl;
    if (!group) return;
    for (const clone of Array.from(group.querySelectorAll(`.${ARROW_OUTLINE_CLASS}`))) {
      const source = clone[ARROW_SOURCE_EXPANDO];
      if (needed === 0 || !source || !source.isConnected) clone.remove();
    }
    if (needed === 0) return;
    const arrowHeads = Array.from(group.querySelectorAll(`.canvas-path-end`)).filter((el) => !el.classList.contains(ARROW_OUTLINE_CLASS));
    for (const arrowHead of arrowHeads) {
      let clones = (_a = arrowHead[ARROW_CLONES_EXPANDO]) != null ? _a : arrowHead[ARROW_CLONES_EXPANDO] = [];
      clones = clones.filter((clone) => clone.isConnected);
      arrowHead[ARROW_CLONES_EXPANDO] = clones;
      while (clones.length > needed) clones.pop().remove();
      while (clones.length < needed) {
        const clone = arrowHead.cloneNode(false);
        clone.removeAttribute("id");
        clone.classList.add(ARROW_OUTLINE_CLASS);
        clone[ARROW_SOURCE_EXPANDO] = arrowHead;
        arrowHead.parentElement.insertBefore(clone, arrowHead);
        clones.push(clone);
      }
      clones.forEach((clone, index) => {
        clone.classList.toggle(ARROW_OUTER_OUTLINE_CLASS, needed === 2 && index === 0);
        for (const attr of ["points", "transform"]) {
          const value = arrowHead.getAttribute(attr);
          if (clone.getAttribute(attr) === value) continue;
          if (value === null) clone.removeAttribute(attr);
          else clone.setAttribute(attr, value);
        }
      });
    }
  }
...
var NodeTemplatesCanvasExtension = class extends CanvasExtension {
  ...
  async createNodeFromTemplate(canvas, template, pos) {
    ...
    if (template.type === "text") node = canvas.createTextNode(creationOptions);
      ...
      if (template.path) {
        ...
        if (abstractFile instanceof import_obsidian18.TFile) tfile = abstractFile;
      }
  ...
  async saveNodeAsTemplate(canvas) {
    ...
    if (!icon) {
      new import_obsidian18.Notice("No icon selected, template creation cancelled.");
      return;
    }
...
var IconModal = class extends import_obsidian18.FuzzySuggestModal {
  constructor(app) {
    super(app);
    this.setPlaceholder("Set template icon");
  }
  getItems() {
    return (0, import_obsidian18.getIconIds)();
  }
  getItemText(item) {
    return item;
  }
  renderSuggestion(item, el) {
    ...
    el.createEl("span", { cls: "icon-modal-suggestion-icon" }, (iconEl) => {
      (0, import_obsidian18.setIcon)(iconEl, item.item);
    });
    el.createEl("span", {
...
var PresentationCanvasExtension = class extends CanvasExtension {
  constructor() {
    ...
    // Added by an LLM agent (P1): per-canvas sessions + animation generation so
    // two leaves presenting don't clobber each other and mashing Right arrow
    // doesn't stack 5 overlapping zoom chains. Legacy singleton fields above are
    // kept as aliases of the active canvas session for API compatibility.
  ...
  async animateNodeTransition(canvas, fromNode, toNode) {
    ...
    const generation = session ? session.animationGen + 1 : 1;
    if (session) session.animationGen = generation;
    const isCurrent = () => {
      var _a;
      return ((_a = this.sessions.get(canvas)) == null ? void 0 : _a.animationGen) === generation;
    };
    ...
    this.plugin.app.workspace.trigger("advanced-canvas:presentation-node-changed", canvas, toNode);
  }
  async startPresentation(canvas, tryContinue = false) {
    let existing = this.sessions.get(canvas);
    if (!existing) {
      existing = { savedViewport: { x: 0, y: 0, zoom: 1 }, visitedNodeIds: [], animationGen: 0, keyHandler: null, fullscreenHandler: null };
      this.sessions.set(canvas, existing);
    }
    const priorVisited = tryContinue ? existing.visitedNodeIds.length > 0 ? existing.visitedNodeIds : this.visitedNodeIds : [];
    if (!tryContinue || priorVisited.length === 0) {
      const startNode2 = canvas.metadata["startNode"] && canvas.nodes.get(canvas.metadata["startNode"]);
      if (!startNode2) {
        new import_obsidian19.Notice("No start node found. Please mark a node as a start node through the popup menu.");
        return;
      }
      this.visitedNodeIds = [startNode2.getData().id];
    } else {
      this.visitedNodeIds = [...priorVisited];
    }
    existing.visitedNodeIds = [...this.visitedNodeIds];
    this.savedViewport = {
      x: canvas.tx,
      y: canvas.ty,
      zoom: canvas.tZoom
    };
    existing.savedViewport = { ...this.savedViewport };
    ...
    if (!session) {
      session = { savedViewport: { x: 0, y: 0, zoom: 1 }, visitedNodeIds: [], animationGen: 0, keyHandler: null, fullscreenHandler: null };
      this.sessions.set(canvas, session);
    }
    if (session.keyHandler) canvas.wrapperEl.removeEventListener("keydown", session.keyHandler);
    const keyHandler = (e) => {
      if (e.key === "Escape") {
        e.preventDefault();
        e.stopPropagation();
    ...
    canvas.wrapperEl.addEventListener("keydown", keyHandler);
    if (this.presentationUsesFullscreen) {
      this.fullscreenModalObserver = new MutationObserver((mutationRecords) => {
        mutationRecords.forEach((mutationRecord) => {
          mutationRecord.addedNodes.forEach((node) => {
            ...
            const el = node;
            if (typeof el.matches !== "function" || !el.matches(".modal-container, .prompt, .menu, .suggestion-container")) return;
            try {
      ...
      if (session.fullscreenHandler) canvas.wrapperEl.removeEventListener("fullscreenchange", session.fullscreenHandler);
      const fullscreenHandler = (_e) => {
        if (activeDocument.fullscreenElement) return;
        this.endPresentation(canvas);
      };
      session.fullscreenHandler = fullscreenHandler;
      canvas.wrapperEl.addEventListener("fullscreenchange", fullscreenHandler);
    }
  ...
  endPresentation(canvas) {
    ...
    const session = this.sessions.get(canvas);
    if (session) {
      session.animationGen++;
      if (session.keyHandler) {
        canvas.wrapperEl.removeEventListener("keydown", session.keyHandler);
        session.keyHandler = null;
      }
      if (session.fullscreenHandler) {
        canvas.wrapperEl.removeEventListener("fullscreenchange", session.fullscreenHandler);
        session.fullscreenHandler = null;
      }
    }
    if (this.presentationUsesFullscreen) {
    ...
    this.isPresentationMode = false;
    this.presentationUsesFullscreen = false;
    this.plugin.app.workspace.trigger("advanced-canvas:presentation-ended", canvas);
  }
  // Added by an LLM agent
  ...
  nextNode(canvas) {
    ...
    if (toNode) {
      ...
      const session = this.sessions.get(canvas);
      if (session) session.visitedNodeIds = [...this.visitedNodeIds];
  ...
  dispose() {
    var _a;
    for (const [, session] of this.sessions) session.animationGen++;
    this.sessions.clear();
    try {
      (_a = this.fullscreenModalObserver) == null ? void 0 : _a.disconnect();
    } catch (e) {
...
var ZOrderingCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "canvas:edge-menu",
      (menu, canvas) => this.edgeContextMenu(canvas, menu)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:edge-added",
      (canvas) => this.reorderEdges(canvas)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:edge-changed",
      (canvas) => this.reorderEdges(canvas)
  ...
  // Added by an LLM agent
  edgeContextMenu(canvas, menu) {
    this.addZOrderingContextMenuItems(canvas, [], this.getSelectedEdges(canvas), menu);
  }
  selectionContextMenu(canvas, menu) {
    const selectedNodes = canvas.getSelectionData().nodes.map((nodeData) => canvas.nodes.get(nodeData.id)).filter((node) => node !== void 0);
    this.addZOrderingContextMenuItems(canvas, selectedNodes, this.getSelectedEdges(canvas), menu);
  }
  // Added by an LLM agent
  getSelectedEdges(canvas) {
    return [...canvas.selection].filter((item) => item.path !== void 0);
  }
  addZOrderingContextMenuItems(canvas, nodes, edges, menu) {
    menu.addSeparator();
    if (this.plugin.settings.getSetting("zOrderingControlShowOneLayerShiftOptions") && nodes.length + edges.length === 1) {
      menu.addItem((item) => {
        item.setTitle("Move one layer forward");
        item.setIcon("arrow-up");
        item.onClick(() => nodes.length === 1 ? this.moveOneLayer(canvas, nodes.first(), true) : this.moveOneEdgeLayer(canvas, edges.first(), true));
      });
      menu.addItem((item) => {
        item.setTitle("Move one layer backward");
        item.setIcon("arrow-down");
        item.onClick(() => nodes.length === 1 ? this.moveOneLayer(canvas, nodes.first(), false) : this.moveOneEdgeLayer(canvas, edges.first(), false));
      });
    }
    menu.addItem((item) => {
      item.setTitle("Bring to Front");
      item.setIcon("bring-to-front");
      item.onClick(() => this.moveMaxLayers(canvas, nodes, edges, true));
    });
    menu.addItem((item) => {
      item.setTitle("Send to Back");
      item.setIcon("send-to-back");
      item.onClick(() => this.moveMaxLayers(canvas, nodes, edges, false));
    });
    if (nodes.some((node) => node.getData().zIndex !== void 0) || edges.some((edge) => this.getEdgeZIndexAttribute(edge) !== void 0)) {
      menu.addItem((item) => {
        item.setTitle("Remove persistent z-index");
        item.setIcon("pin-off");
        item.onClick(() => this.removePersistentZIndexes(canvas, nodes, edges));
      });
    }
    menu.addSeparator();
  }
  ...
  // Added by an LLM agent
  moveOneEdgeLayer(canvas, selectedEdge, forward) {
    const selectedEdgeBBox = selectedEdge.getBBox();
    const selectedZIndex = this.getEdgeZIndex(selectedEdge);
    const nearestZIndexEdge = [...canvas.edges.values()].filter((edge) => BBoxHelper.isColliding(selectedEdgeBBox, edge.getBBox())).filter((edge) => edge !== selectedEdge).sort((a, b) => forward ? this.getEdgeZIndex(a) - this.getEdgeZIndex(b) : this.getEdgeZIndex(b) - this.getEdgeZIndex(a)).filter((edge) => forward ? this.getEdgeZIndex(edge) > selectedZIndex : this.getEdgeZIndex(edge) < selectedZIndex).first();
    if (nearestZIndexEdge === void 0) return;
    const targetZIndex = this.getEdgeZIndex(nearestZIndexEdge);
    this.setEdgesZIndex(canvas, [nearestZIndexEdge], selectedZIndex, false);
    this.setEdgesZIndex(canvas, [selectedEdge], targetZIndex, true);
  }
  moveMaxLayers(canvas, selectedNodes, selectedEdges, forward) {
    if (selectedNodes.length > 0) {
      let targetZIndex;
      if (forward) {
        let max = -Infinity;
        for (const node of canvas.nodes.values()) max = Math.max(max, node.zIndex);
        targetZIndex = max + 1;
      } else {
        let min = Infinity;
        for (const node of canvas.nodes.values()) min = Math.min(min, node.zIndex);
        targetZIndex = min - selectedNodes.length;
      }
    ...
    if (selectedEdges.length > 0) {
      ...
      if (forward) {
        let max = -Infinity;
        for (const edge of canvas.edges.values()) max = Math.max(max, this.getEdgeZIndex(edge));
        targetZIndex = max + 1;
      } else {
        let min = Infinity;
        for (const edge of canvas.edges.values()) min = Math.min(min, this.getEdgeZIndex(edge));
        targetZIndex = min - selectedEdges.length;
      }
      this.setEdgesZIndex(canvas, selectedEdges, targetZIndex);
    }
  }
  removePersistentZIndexes(canvas, nodes, edges) {
    for (const node of nodes) node.setZIndex(void 0);
    if (edges.length > 0) this.setEdgesZIndex(canvas, edges, null);
  }
  setNodesZIndex(nodes, zIndex) {
    ...
    this.maybeCompactNodeZIndexes((_a = nodes.first()) == null ? void 0 : _a.canvas);
  }
  /** Added by an LLM agent (P2-16): renormalize persistent zIndexes to 1..N. */
  maybeCompactNodeZIndexes(canvas) {
    if (!canvas) return;
    let max = -Infinity;
    for (const node of canvas.nodes.values()) max = Math.max(max, node.zIndex);
    if (max < 1e5) return;
    const sorted = [...canvas.nodes.values()].sort((a, b) => a.zIndex - b.zIndex);
    sorted.forEach((node, i) => {
      const data = node.getData();
      if (data.zIndex !== void 0) node.setZIndex(i + 1);
    });
    canvas.zIndexCounter = sorted.length + 1;
  }
  // Added by an LLM agent: persists the zIndex in the edge's styleAttributes (as a string, like every style attribute)
  setEdgesZIndex(canvas, edges, zIndex, pushHistory = true) {
    const withOrder = edges.map((edge) => ({ edge, order: this.getEdgeZIndex(edge) }));
    withOrder.sort((a, b) => a.order - b.order);
    for (let i = 0; i < withOrder.length; i++) {
      const edge = withOrder[i].edge;
      const edgeData = edge.getData();
      const finalZIndex = zIndex === null ? null : zIndex + i;
      edge.setData({
        ...edgeData,
        styleAttributes: {
          ...edgeData.styleAttributes,
          zIndex: finalZIndex === null ? null : finalZIndex.toString()
        }
      }, false);
    }
    if (pushHistory) canvas.pushHistory(canvas.getData());
    this.reorderEdges(canvas);
  }
  // Added by an LLM agent: SVG <g> elements ignore CSS z-index, so edge stacking is done via DOM order inside the edge containers
  reorderEdges(canvas) {
    var _a;
    const edges = [...canvas.edges.values()];
    const sortedEdges = edges.map((edge) => ({ edge, order: this.getEdgeZIndex(edge) })).sort((a, b) => a.order - b.order);
    if (!sortedEdges.some((item) => item.order !== 0)) return;
    let dirty = false;
    for (let i = 0; i < sortedEdges.length; i++) {
      const lineGroup = sortedEdges[i].edge.lineGroupEl;
      if (!lineGroup || ((_a = lineGroup.parentElement) == null ? void 0 : _a.children[i]) !== lineGroup) {
        dirty = true;
        break;
      }
    }
    ...
    for (const { edge } of sortedEdges) {
      if (!edge.isAttached) continue;
      canvas.edgeContainerEl.appendChild(edge.lineGroupEl);
      canvas.edgeEndContainerEl.appendChild(edge.lineEndGroupEl);
    }
  }
  // Added by an LLM agent
  getEdgeZIndexAttribute(edge) {
    var _a, _b;
    return (_b = (_a = edge.getData().styleAttributes) == null ? void 0 : _a.zIndex) != null ? _b : void 0;
  }
  // Added by an LLM agent
  getEdgeZIndex(edge) {
    const parsed = Number(this.getEdgeZIndexAttribute(edge));
    return isNaN(parsed) ? 0 : parsed;
  }
  // Added by an LLM agent
  getAllEdgeZIndexes(canvas) {
    return [...canvas.edges.values()].map((edge) => this.getEdgeZIndex(edge));
  }
  getAllNodeZIndexes(canvas) {
...
var CopyNodeReferenceCanvasExtension = class _CopyNodeReferenceCanvasExtension extends CanvasExtension {
  ...
  static copyWikilinkToNode(file, nodeData) {
    ...
    navigator.clipboard.writeText(wikilink).then(
      () => new import_obsidian20.Notice("Copied wikilink to node to clipboard.", 2e3)
    ).catch(() => new import_obsidian20.Notice("Failed to copy wikilink to node to clipboard.", 2e3));
  }
};
        ...
        (canvas) => {
          ...
          for (const nodeId of selectedNodeIds) {
            ...
            for (const nodeOutgoingLink of nodeOutgoingLinks) {
              ...
              if (!(resolvedLink instanceof import_obsidian21.TFile)) continue;
      ...
      CanvasHelper.createControlMenuButton({
        ...
        callback: () => {
          ...
          if (!(propertiesPlugin == null ? void 0 : propertiesPlugin._loaded)) {
            new import_obsidian22.Notice(`Core plugin "Properties view" was not found or isn't enabled. Enable it and restart Obsidian.`);
            return;
          }
...
var CollapsibleGroupsCanvasExtension = class extends CanvasExtension {
  ...
  onNodeChanged(canvas, groupNode) {
    ...
    (0, import_obsidian23.setIcon)(collapseEl, groupNodeData.collapsed ? "plus-circle" : "minus-circle");
...
var MAX_EXPORT_PIXELS = 16e3 * 16e3;
var MAX_EXPORT_DIMENSION = 16e3;
var ExportCanvasExtension = class extends CanvasExtension {
  ...
  async showExportImageSettingsModal(canvas, nodesToExport) {
    ...
    let svg = false;
    new import_obsidian24.Setting(modal.contentEl).setName("Export file format").setDesc("Choose the file format to export the canvas as.").addDropdown(
    ...
    pixelRatioSetting = new import_obsidian24.Setting(modal.contentEl).setName("Pixel ratio").setDesc("Higher pixel ratios result in higher resolution images but also larger file sizes.").addSlider(
    ...
    noFontExportSetting = new import_obsidian24.Setting(modal.contentEl).setName("Skip font export").setDesc("This will not include the fonts in the exported SVG. This will make the SVG file smaller.").addToggle(
    ...
    new import_obsidian24.Setting(modal.contentEl).setName("Theme").setDesc("The theme used for the export.").addDropdown(
      (dropdown) => dropdown.addOptions({
        light: "Light",
        dark: "Dark"
    ...
    let watermark = false;
    new import_obsidian24.Setting(modal.contentEl).setName("Show logo").setDesc("This will add an Obsidian + Advanced Canvas logo to the bottom left.").addToggle(
      (toggle) => toggle.setValue(watermark).onChange((value) => watermark = value)
    );
    let garbledText = false;
    new import_obsidian24.Setting(modal.contentEl).setName("Privacy mode").setDesc("This will obscure any text on your canvas.").addToggle(
      (toggle) => toggle.setValue(garbledText).onChange((value) => garbledText = value)
    );
    let transparentBackground = false;
    transparentBackgroundSetting = new import_obsidian24.Setting(modal.contentEl).setName("Transparent background").setDesc("This will make the background of the image transparent.").addToggle(
      (toggle) => toggle.setValue(transparentBackground).onChange((value) => transparentBackground = value)
    );
    new import_obsidian24.Setting(modal.contentEl).addButton(
  ...
  async exportImage(canvas, nodesToExport, svg, pixelRatioFactor, noFontExport, theme, watermark, garbledText, transparentBackground) {
    ...
    const nodesToExportIds = new Set(nodesToExport.map((node) => node.getData().id));
    const edgesToExport = [...canvas.edges.values()].filter((edge) => {
      const edgeData = edge.getData();
      return nodesToExportIds.has(edgeData.fromNode) && nodesToExportIds.has(edgeData.toNode);
    });
    ...
    new import_obsidian24.Notice("Exporting the canvas. Please wait...");
    ...
    try {
      ...
      let pixelRatio = svg ? void 0 : Math.round(requiredPixelRatio * pixelRatioFactor);
      if (pixelRatio !== void 0) {
        const estWidth = enlargedTargetBoundingBoxSize.width * pixelRatio;
        const estHeight = enlargedTargetBoundingBoxSize.height * pixelRatio;
        const estPixels = estWidth * estHeight;
        if (estPixels > MAX_EXPORT_PIXELS || estWidth > MAX_EXPORT_DIMENSION || estHeight > MAX_EXPORT_DIMENSION) {
          const scale = Math.min(
            Math.sqrt(MAX_EXPORT_PIXELS / Math.max(estPixels, 1)),
            MAX_EXPORT_DIMENSION / Math.max(estWidth, 1),
            MAX_EXPORT_DIMENSION / Math.max(estHeight, 1)
          );
          pixelRatio = Math.max(1, Math.floor(pixelRatio * scale));
          new import_obsidian24.Notice(`Export resolution capped to ${pixelRatio}x to avoid running out of memory.`);
        }
      }
      ...
      const edgePathsBBox = BBoxHelper.combineBBoxes(edgesToExport.map((edge) => {
        ...
        let labelWidth = 0;
        try {
          const rect = (_b2 = (_a2 = edge.labelElement) == null ? void 0 : _a2.wrapperEl) == null ? void 0 : _b2.getBoundingClientRect();
          if (rect) labelWidth = rect.width / canvasScale;
        } catch (e) {
        }
      ...
      if (unloadedNodes.length === 0) {
        const nodeElements = new Set(nodesToExport.map((node) => node.nodeEl));
        const edgePathAndArrowElements = new Set(edgesToExport.map((edge) => [edge.lineGroupEl, edge.lineEndGroupEl]).flat());
        const edgeLabelElements = new Set(edgesToExport.map((edge) => {
          var _a2;
          return (_a2 = edge.labelElement) == null ? void 0 : _a2.wrapperEl;
        }).filter((labelElement) => labelElement !== void 0));
        const filter = (element) => {
          var _a2, _b2, _c2, _d;
          if (((_a2 = element.classList) == null ? void 0 : _a2.contains("canvas-node")) && !nodeElements.has(element))
            return false;
          if (((_c2 = (_b2 = element.parentElement) == null ? void 0 : _b2.classList) == null ? void 0 : _c2.contains("canvas-edges")) && !edgePathAndArrowElements.has(element))
            return false;
          if (((_d = element.classList) == null ? void 0 : _d.contains("canvas-path-label-wrapper")) && !edgeLabelElements.has(element))
            return false;
          return true;
        };
        const options = {
        ...
        new import_obsidian24.Notice(ERROR_MESSAGE);
...
var EdgeLabelMarkdownCanvasExtension = class extends CanvasExtension {
  ...
  async renderMarkdown(labelElement) {
    ...
    try {
      await import_obsidian25.MarkdownRenderer.render(this.plugin.app, source, textareaEl, sourcePath, component);
    } catch (error) {
  ...
  resetRenderComponent(labelElement) {
    this.unloadRenderComponent(labelElement);
    const component = new import_obsidian25.Component();
...
// src/canvas-extensions/bezier-curvature-canvas-extension.ts
function scaledControlPoint(anchor, controlPoint, factor) {
  return {
    x: anchor.x + (controlPoint.x - anchor.x) * factor,
    y: anchor.y + (controlPoint.y - anchor.y) * factor
  };
}
var BezierCurvatureCanvasExtension = class extends CanvasExtension {
  isEnabled() {
    return true;
  }
  init() {
  ...
  onEdgePathUpdated(_canvas, edge) {
    var _a, _b;
    if (!this.plugin.settings.getSetting("edgesStylingFeatureEnabled")) return;
    if (!edge.bezier || !((_a = edge.path) == null ? void 0 : _a.display)) return;
    const styleAttributes = (_b = edge.getData()) == null ? void 0 : _b.styleAttributes;
    if (styleAttributes == null ? void 0 : styleAttributes.pathfindingMethod) return;
    const rawCurvature = styleAttributes == null ? void 0 : styleAttributes.bezierCurvature;
    if (rawCurvature === void 0 || rawCurvature === null) return;
    const curvature = Number.parseFloat(rawCurvature);
    if (!Number.isFinite(curvature) || curvature < 0 || curvature === 1) return;
    const { from, to, cp1, cp2 } = edge.bezier;
    const scaledCp1 = scaledControlPoint(from, cp1, curvature);
    const scaledCp2 = scaledControlPoint(to, cp2, curvature);
    const curve = `M${from.x},${from.y} C${scaledCp1.x},${scaledCp1.y} ${scaledCp2.x},${scaledCp2.y} ${to.x},${to.y}`;
    Object.assign(edge.bezier, { cp1: scaledCp1, cp2: scaledCp2, path: curve });
    let path = curve;
    if (!edge.fromLineEnd) {
      const fromBorder = BBoxHelper.getCenterOfBBoxSide(edge.from.node.getBBox(), edge.from.side);
  ...
  addRotationMenuItems(canvas, nodes, menu) {
    ...
    menu.addItem((item) => {
      ...
      item.onClick(async () => {
        ...
        if (isNaN(angle)) {
          new import_obsidian26.Notice("Invalid rotation angle");
          return;
        }
...
// src/canvas-extensions/hide-node-contents-canvas-extension.ts
var HIDE_CONTENTS_CLASS = "ac-node-contents-hidden";
var HideNodeContentsCanvasExtension = class extends CanvasExtension {
  isEnabled() {
    return "hideNodeContentsFeatureEnabled";
  }
  init() {
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:node-rendered",
      (_canvas, node) => this.updateNodeEl(node)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:data-loaded:after",
      (canvas) => {
        for (const node of canvas.nodes.values()) this.updateNodeEl(node);
      }
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "canvas:node-menu",
      (menu, node) => this.addMenuItem(node.canvas, [node], menu)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "canvas:selection-menu",
      (menu, canvas) => this.addMenuItem(canvas, this.getSelectedNodes(canvas), menu)
    ));
  }
  getSelectedNodes(canvas) {
    return canvas.getSelectionData().nodes.map((nodeData) => canvas.nodes.get(nodeData.id)).filter((node) => node !== void 0);
  }
  addMenuItem(canvas, nodes, menu) {
    if (canvas.readonly) return;
    nodes = nodes.filter((node) => node.getData().type !== "group");
    if (nodes.length === 0) return;
    const allHidden = nodes.every((node) => node.getData().hideContents === true);
    menu.addItem((item) => {
      item.setTitle(allHidden ? "Show node contents" : "Hide node contents");
      item.setIcon(allHidden ? "eye" : "eye-off");
      item.onClick(() => this.setContentsHidden(canvas, nodes, !allHidden));
    });
  }
  setContentsHidden(canvas, nodes, hidden) {
    for (const node of nodes) {
      node.setData({
        ...node.getData(),
        hideContents: hidden ? true : void 0
        // Keep the canvas JSON clean
      });
  ...
  updateNodeEl(node) {
    node.nodeEl.classList.toggle(HIDE_CONTENTS_CLASS, node.getData().hideContents === true);
  }
};

// src/canvas-extensions/text-wrap-around-canvas-extension.ts
var TEXT_WRAP_AROUND_CLASS = "ac-text-wrap-around";
var TEXT_WRAP_AROUND_SPACER_CLASS = "ac-text-wrap-spacer";
var WRAP_GAP_PX = 8;
var WRAPPABLE_NODE_TYPES = ["text", "file"];
var TextWrapAroundCanvasExtension = class extends CanvasExtension {
  constructor() {
    super(...arguments);
    /** Canvases with a recompute queued for the next animation frame */
    this.pendingCanvases = /* @__PURE__ */ new Set();
    this.rafScheduled = false;
    /** Capture-phase scroll listeners, one per canvas (removed on canvas unload / dispose) */
    this.scrollCleanups = /* @__PURE__ */ new Map();
  }
  isEnabled() {
    return "textWrapAroundFeatureEnabled";
  }
  init() {
    const refreshEvents = [
      "advanced-canvas:node-rendered",
    ...
    for (const event of refreshEvents) {
      this.plugin.registerEvent(this.plugin.app.workspace.on(
        event,
        // Rest params: these events carry extra payloads (node, update, ...) that the
        // refresh does not need - one handler shape fits the whole overload union.
        (canvas, ..._rest) => this.schedule(canvas)
      ));
    }
    for (const event of ["advanced-canvas:data-loaded:after", "advanced-canvas:canvas-changed", "advanced-canvas:undo", "advanced-canvas:redo", "advanced-canvas:viewport-changed:after", "advanced-canvas:edge-added", "advanced-canvas:edge-changed", "advanced-canvas:edge-removed"]) {
      this.plugin.registerEvent(this.plugin.app.workspace.on(
        event,
        (canvas) => this.schedule(canvas)
      ));
    }
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:canvas-view-unloaded:before",
      (view) => this.removeScrollListener(view.canvas)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "canvas:node-menu",
      (menu, node) => this.addMenuItems(node.canvas, [node], menu)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "canvas:selection-menu",
      (menu, canvas) => this.addMenuItems(canvas, this.getSelectedNodes(canvas), menu)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:popup-menu-created",
      (canvas) => this.onPopupMenuCreated(canvas)
  ...
  dispose() {
    ...
    for (const cleanup of this.scrollCleanups.values()) {
      try {
        cleanup();
      } catch (e) {
      }
    }
    this.scrollCleanups.clear();
    for (const canvas of this.plugin.getCanvases()) this.clearCanvas(canvas);
  }
  onSettingsEnabled() {
    for (const canvas of this.plugin.getCanvases()) this.schedule(canvas);
  }
  getSelectedNodes(canvas) {
    return canvas.getSelectionData().nodes.map((nodeData) => canvas.nodes.get(nodeData.id)).filter((node) => node !== void 0);
  }
  isWrappable(node) {
    return WRAPPABLE_NODE_TYPES.includes(node.getData().type);
  }
  isWrapEnabled(node) {
    return node.getData().textWrapAround === true;
  }
  // #region Menus
  addMenuItems(canvas, nodes, menu) {
    if (!this.isActive() || canvas.readonly) return;
    nodes = nodes.filter((node) => this.isWrappable(node));
    if (nodes.length === 0) return;
    const allEnabled = nodes.every((node) => this.isWrapEnabled(node));
    menu.addItem((item) => {
      item.setTitle("Wrap text around overlapping nodes");
      item.setIcon("wrap-text");
      item.setChecked(allEnabled);
      item.onClick(() => this.setNodesWrap(canvas, nodes, !allEnabled));
    });
  }
  onPopupMenuCreated(canvas) {
    if (!this.isActive() || canvas.readonly) return;
    const nodes = this.getSelectedNodes(canvas).filter((node) => this.isWrappable(node));
    if (nodes.length === 0) return;
    const allEnabled = nodes.every((node) => this.isWrapEnabled(node));
    const button = CanvasHelper.createPopupMenuOption({
      id: "ac-text-wrap-around",
      label: "Wrap text around overlapping nodes",
      icon: "wrap-text",
      callback: () => {
        const next = !nodes.every((node) => this.isWrapEnabled(node));
        this.setNodesWrap(canvas, nodes, next);
        button.classList.toggle("is-active", next);
      }
    });
    button.classList.toggle("is-active", allEnabled);
    CanvasHelper.addPopupMenuOption(canvas, button);
  }
  setNodesWrap(canvas, nodes, enabled) {
    for (const node of nodes) {
      node.setData({
        ...node.getData(),
        textWrapAround: enabled ? true : void 0
        // Keep the canvas JSON clean
      });
    }
    canvas.pushHistory(canvas.getData());
  ...
  schedule(canvas) {
    ...
    this.pendingCanvases.add(canvas);
    this.ensureScrollListener(canvas);
    if (this.rafScheduled) return;
    this.rafScheduled = true;
    window.requestAnimationFrame(() => {
      this.rafScheduled = false;
      const canvases = [...this.pendingCanvases];
      this.pendingCanvases.clear();
      for (const pending of canvases) {
        try {
          this.refreshCanvas(pending);
        } catch (e) {
          console.error("Advanced Canvas: text wrap-around refresh failed:", e);
        }
      }
    });
  }
  /** Content scrolling moves overlaps relative to the in-flow spacers - recompute on scroll */
  ensureScrollListener(canvas) {
    if (this.scrollCleanups.has(canvas) || !canvas.canvasEl) return;
    const onScroll = () => this.schedule(canvas);
    canvas.canvasEl.addEventListener("scroll", onScroll, true);
    this.scrollCleanups.set(canvas, () => canvas.canvasEl.removeEventListener("scroll", onScroll, true));
  }
  removeScrollListener(canvas) {
    const cleanup = this.scrollCleanups.get(canvas);
    if (!cleanup) return;
    this.scrollCleanups.delete(canvas);
    try {
      cleanup();
    } catch (e) {
    }
  ...
  refreshCanvas(canvas) {
    ...
    const linkedObstacles = this.buildLinkedObstacleMap(canvas);
    for (const node of canvas.nodes.values()) {
      const enabled = this.isWrappable(node) && this.isWrapEnabled(node);
      node.nodeEl.classList.toggle(TEXT_WRAP_AROUND_CLASS, enabled);
      if (!enabled) {
        this.clearSpacers(node);
        continue;
      }
      this.updateNode(canvas, node, linkedObstacles.get(node.id));
    }
  }
  clearCanvas(canvas) {
    for (const node of canvas.nodes.values()) {
      node.nodeEl.classList.remove(TEXT_WRAP_AROUND_CLASS);
      this.clearSpacers(node);
    }
  }
  clearSpacers(node) {
    if (!node.contentEl) return;
    node.contentEl.querySelectorAll(`:scope .${TEXT_WRAP_AROUND_SPACER_CLASS}`).forEach((spacer) => spacer.remove());
  }
  findMarkdownContainer(node) {
    return node.contentEl.querySelector(":scope .markdown-preview-view .markdown-preview-section");
  }
  updateNode(canvas, node, linkedObstacleIds) {
    var _a, _b;
    if (!node.contentEl) return;
    const container = this.findMarkdownContainer(node);
    if (!container) {
      this.clearSpacers(node);
      return;
    }
    if (!linkedObstacleIds || linkedObstacleIds.size === 0) {
      this.clearSpacers(node);
      return;
    }
    const nodeRect = node.nodeEl.getBoundingClientRect();
    if (node.width <= 0 || nodeRect.width <= 0) {
      this.clearSpacers(node);
      return;
    }
    const scale = nodeRect.width / node.width;
    const containerRect = container.getBoundingClientRect();
    if (containerRect.width <= 0 || containerRect.height <= 0) {
      this.clearSpacers(node);
      return;
    }
    const visible = this.visibleSectionRect(container, node, scale);
    if (!visible) return;
    const computed = window.getComputedStyle(container);
    const padLeft = parseFloat(computed.paddingLeft) || 0;
    const padTop = parseFloat(computed.paddingTop) || 0;
    const originX = containerRect.left + padLeft * scale;
    const originY = containerRect.top + padTop * scale;
    const contentWidth = container.clientWidth - padLeft - (parseFloat(computed.paddingRight) || 0);
    this.clearSpacers(node);
    const contentHeight = container.scrollHeight - padTop - (parseFloat(computed.paddingBottom) || 0);
    if (contentWidth <= 0 || contentHeight <= 0) return;
    const bands = this.computeBands(canvas, node, { originX, originY, scale, contentWidth, contentHeight, visible }, linkedObstacleIds);
    if (bands.length === 0) return;
    const childBottoms = Array.from(container.children).filter((child) => !child.classList.contains(TEXT_WRAP_AROUND_SPACER_CLASS)).map((child) => ({ child, bottom: (child.getBoundingClientRect().bottom - originY) / scale }));
    for (const band of bands) {
      const spacer = activeDocument.createElement("div");
      spacer.classList.add(TEXT_WRAP_AROUND_SPACER_CLASS);
      const height = band.y1 - band.y0;
      const useLeftSide = band.x0 >= contentWidth - band.x1;
      if (useLeftSide) {
        spacer.style.float = "right";
        spacer.style.width = `${Math.max(0, contentWidth - band.x0)}px`;
        spacer.style.marginLeft = `${WRAP_GAP_PX}px`;
      } else {
        spacer.style.float = "left";
        spacer.style.width = `${Math.max(0, band.x1)}px`;
        spacer.style.marginRight = `${WRAP_GAP_PX}px`;
      }
      spacer.style.height = `${height}px`;
      const anchor = (_b = (_a = childBottoms.find(({ bottom }) => bottom > band.y0 + 1)) == null ? void 0 : _a.child) != null ? _b : null;
      container.insertBefore(spacer, anchor);
    }
  }
  /**
   * The section's box clipped to every clipping ancestor up to the node frame, in screen
   * px. Returns null when nothing of the section is visible.
   *
   * Added by an LLM agent: `clientWidth`/`clientHeight`/`clientLeft`/`clientTop` are
   * layout px (unaffected by the canvas zoom transform) while every rect and the
   * accumulated region are screen px, so the client metrics must be scaled - unscaled,
   * the visible region inflates by 1/scale (verified live: at 0.38x zoom the bottom
   * edge overshot by ~430 screen px, forging phantom bands from nodes far below and
   * beside the wrap node).
   */
  visibleSectionRect(section, node, scale) {
    const r = section.getBoundingClientRect();
    const region = { left: r.left, top: r.top, right: r.right, bottom: r.bottom };
    let el = section.parentElement;
    while (el) {
      const style = window.getComputedStyle(el);
      if (style.overflowX !== "visible" || style.overflowY !== "visible") {
        const b = el.getBoundingClientRect();
        const left = b.left + el.clientLeft * scale;
        const top = b.top + el.clientTop * scale;
        region.left = Math.max(region.left, left);
        region.top = Math.max(region.top, top);
        region.right = Math.min(region.right, left + el.clientWidth * scale);
        region.bottom = Math.min(region.bottom, top + el.clientHeight * scale);
        if (region.right <= region.left || region.bottom <= region.top) return null;
      }
      if (el === node.nodeEl) break;
      el = el.parentElement;
    }
    return region;
  }
  /**
   * Added by an LLM agent: nodes allowed to act as wrap obstacles for each wrap node,
   * keyed by wrap node id. Only nodes with a bidirectional edge going INTO the wrap
   * node count: an edge whose `toNode` is the wrap node and whose `fromEnd` is
   * `'arrow'`; the node at the edge's start (`fromNode`) is the obstacle.
   */
  buildLinkedObstacleMap(canvas) {
    const map = /* @__PURE__ */ new Map();
    for (const edge of canvas.edges.values()) {
      const data = edge.getData();
      if (data.fromEnd !== "arrow" || !data.fromNode || !data.toNode) continue;
      let set = map.get(data.toNode);
      if (!set) {
        set = /* @__PURE__ */ new Set();
        map.set(data.toNode, set);
      }
      set.add(data.fromNode);
    }
    return map;
  }
  /**
   * Overlaps of nodes painting above `node`, merged into disjoint Y bands in the node's
   * local content-box px. Each band's X span is the min..max union of its overlaps.
   * Only edge-linked obstacles (see buildLinkedObstacleMap) are measured - every other
   * node is skipped before any geometry read. Overlaps are clipped to the section's
   * visible region, so only obstacles covering on-screen text produce bands.
   */
  computeBands(canvas, node, geometry, linkedObstacleIds) {
    const { originX, originY, scale, contentWidth, contentHeight, visible } = geometry;
    const overlaps = [];
    for (const other of canvas.nodes.values()) {
      if (other === node || !linkedObstacleIds.has(other.id) || !this.isOnTop(other, node)) continue;
      const otherRect = other.nodeEl.getBoundingClientRect();
      const x0 = Math.max(otherRect.left, visible.left);
      const x1 = Math.min(otherRect.right, visible.right);
      const y0 = Math.max(otherRect.top, visible.top);
      const y1 = Math.min(otherRect.bottom, visible.bottom);
      const local = {
        x0: Math.max(0, (x0 - originX) / scale),
        x1: Math.min(contentWidth, (x1 - originX) / scale),
        y0: Math.max(0, (y0 - originY) / scale),
        y1: Math.min(contentHeight, (y1 - originY) / scale)
      };
      if (local.x1 - local.x0 < 1 || local.y1 - local.y0 < 1) continue;
      overlaps.push(local);
    }
    overlaps.sort((a, b) => a.y0 - b.y0 || a.x0 - b.x0);
    const bands = [];
    for (const overlap of overlaps) {
      const current = bands[bands.length - 1];
      if (current && overlap.y0 <= current.y1) {
        current.y1 = Math.max(current.y1, overlap.y1);
        current.x0 = Math.min(current.x0, overlap.x0);
        current.x1 = Math.max(current.x1, overlap.x1);
      } else {
        bands.push({ ...overlap });
      }
    }
  ...
  /**
   * Whether `obstacle` paints above `background`: rendered z-index first, live zIndex
   * counter second, DOM order last (later siblings paint on top at equal z-index).
   */
  isOnTop(obstacle, background) {
    const obstacleZ = parseFloat(obstacle.nodeEl.style.zIndex);
    const backgroundZ = parseFloat(background.nodeEl.style.zIndex);
    if (!Number.isNaN(obstacleZ) && !Number.isNaN(backgroundZ) && obstacleZ !== backgroundZ)
      return obstacleZ > backgroundZ;
    if (obstacle.zIndex !== background.zIndex) return obstacle.zIndex > background.zIndex;
    const position = background.nodeEl.compareDocumentPosition(obstacle.nodeEl);
...
// src/canvas-extensions/presentation-filter-canvas-extension.ts
var import_obsidian27 = require("obsidian");
var NO_FILTER = "none";
var PresentationFilterCanvasExtension = class extends CanvasExtension {
  constructor() {
    super(...arguments);
    // Canvases where this feature currently has a filter applied - used to only reset
    // filters this feature applied, leaving manually applied canvas filters alone
    this.filteredCanvases = /* @__PURE__ */ new WeakSet();
  }
  isEnabled() {
    return true;
  }
  init() {
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "canvas:node-menu",
      (menu, node) => this.addMenuItems(menu, node)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:presentation-node-changed",
      (canvas, node) => this.onSlideChanged(canvas, node)
    ));
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      "advanced-canvas:presentation-ended",
      (canvas) => this.onPresentationEnded(canvas)
    ));
  }
  addMenuItems(menu, node) {
    var _a;
    if (node.canvas.readonly) return;
    const currentFilterId = (_a = node.getData().slideFilter) != null ? _a : this.plugin.settings.getSetting("presentationSlideFilter");
    const options = [{ id: NO_FILTER, name: "None" }, ...NODE_BASED_FILTERS];
    for (const option of options) {
      menu.addItem(
        (item) => item.setTitle(`Slide filter: ${option.name}`).setIcon("filter").setChecked(currentFilterId === option.id).setSection("presentation-slide-filter").onClick(() => {
          node.setData({ ...node.getData(), slideFilter: option.id });
          node.canvas.pushHistory(node.canvas.getData());
          new import_obsidian27.Notice(`Slide filter of node set to: ${option.name}`);
        })
      );
    }
  }
  onSlideChanged(canvas, slideNode) {
    var _a;
    const filterExtension = this.getFilterExtension();
    if (!filterExtension) return;
    const slideId = slideNode.getData().id;
    const targetNodeIds = new Set(canvas.getEdgesForNode(slideNode).map((edge) => edge.getData()).filter((edgeData) => edgeData.toNode === slideId && edgeData.fromEnd === "arrow").map((edgeData) => edgeData.fromNode));
    if (targetNodeIds.size !== 1) return;
    const targetNode = canvas.nodes.get([...targetNodeIds][0]);
    if (!targetNode) return;
    const filterId = (_a = targetNode.getData().slideFilter) != null ? _a : this.plugin.settings.getSetting("presentationSlideFilter");
    if (filterId && filterId !== NO_FILTER) {
      filterExtension.applyNodeBasedFilter(canvas, filterId, [targetNode]);
      this.filteredCanvases.add(canvas);
    } else {
      this.resetOwnFilter(filterExtension, canvas);
    }
  }
  onPresentationEnded(canvas) {
    const filterExtension = this.getFilterExtension();
    if (filterExtension) this.resetOwnFilter(filterExtension, canvas);
  }
  /** Resets the filter only if this feature applied one on the given canvas. */
  resetOwnFilter(filterExtension, canvas) {
    if (!this.filteredCanvases.has(canvas)) return;
    filterExtension.resetFilter(canvas);
    this.filteredCanvases.delete(canvas);
  }
  /**
   * Finds the canvas filter extension instance. Looked up lazily because plugin.canvasExtensions
   * is only assigned after every extension's constructor (and thus init()) has already run.
   * Undefined when the canvas filter feature is disabled.
   */
  getFilterExtension() {
    var _a;
    return (_a = this.plugin.canvasExtensions) == null ? void 0 : _a.find((extension) => extension instanceof CanvasFilterCanvasExtension);
  }
};
...
// src/canvas-extensions/dataset-exposers/dataset-helpers.ts
function isEmptyDatasetValue(value) {
  return value === null || value === void 0 || value === "";
}
function expandDatasetPairs(data, exposedKey) {
  const raw = data[exposedKey];
  const pairs = raw !== null && typeof raw === "object" && !Array.isArray(raw) ? Object.entries(raw) : [[exposedKey, raw]];
  return pairs;
}
function writeDataset(element, data, exposedKeys, keyPrefix = "") {
  const dataset = element.dataset;
  if (!dataset) return;
  for (const exposedKey of exposedKeys) {
    for (const [key, value] of expandDatasetPairs(data, exposedKey)) {
      const datasetKey = keyPrefix ? keyPrefix + key.charAt(0).toUpperCase() + key.slice(1) : key;
      if (isEmptyDatasetValue(value)) delete dataset[datasetKey];
      else dataset[datasetKey] = String(value);
    }
  }
}
...
var NodeExposerExtension = class extends CanvasExtension {
  constructor() {
    super(...arguments);
    // Added by an LLM agent (P0-2): track iframe MutationObservers per node so
    // they can be disconnected on edit-end/unload instead of leaking one per edit.
    this.iframeObservers = /* @__PURE__ */ new Map();
  }
  isEnabled() {
    return true;
  }
  init() {
    ...
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      ...
      (_canvas, node, editing) => {
        var _a, _b;
        if (!editing) {
          this.disconnectIframeObserver(node.id);
          return;
        }
        const nodeData = node.getData();
        if (!nodeData) return;
        ...
        this.disconnectIframeObserver(node.id);
        const observer = new MutationObserver(() => iframe.classList.toggle(CANVAS_NODE_IFRAME_BODY_CLASS, true));
        observer.observe(iframe, { attributes: true, attributeFilter: ["class"] });
        this.iframeObservers.set(node.id, observer);
        ...
        cleanup.observe(node.nodeEl, { childList: true, subtree: true });
        this.plugin.register(() => {
          observer.disconnect();
          cleanup.disconnect();
        });
  ...
  disconnectIframeObserver(nodeId) {
    const existing = this.iframeObservers.get(nodeId);
    if (existing) {
      try {
        existing.disconnect();
      } catch (e) {
      }
      this.iframeObservers.delete(nodeId);
    }
  }
  setDataAttributes(element, nodeData) {
    const exposed = getExposedNodeData(this.plugin.settings);
    if (exposed.length === 0) return;
    writeDataset(element, nodeData, exposed);
  }
};
  ...
  init() {
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      ...
      (canvas, node) => {
        ...
        const exposed = getExposedNodeData(this.plugin.settings);
        if (exposed.length !== 0)
          writeDataset(interactionEl, nodeData, exposed, TARGET_NODE_DATASET_PREFIX);
  ...
  init() {
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      ...
      (_canvas, edge) => {
        ...
        if (!edgeData) return;
        const exposed = getExposedEdgeData(this.plugin.settings);
        if (exposed.length === 0) return;
        const record = edgeData;
        if ((_a = edge.path) == null ? void 0 : _a.display) writeDataset(edge.path.display, record, exposed);
        if ((_b = edge.fromLineEnd) == null ? void 0 : _b.el) writeDataset(edge.fromLineEnd.el, record, exposed);
        if ((_c = edge.toLineEnd) == null ? void 0 : _c.el) writeDataset(edge.toLineEnd.el, record, exposed);
      }
    ));
  }
};
    ...
    Patcher.patchPrototype(this.plugin, cell, {
      render: Patcher.OverrideExisting((next) => function(ctx) {
        ...
        try {
          ...
          if (isCanvas && ctx.file) ctx.file.extension = "canvas";
        }
      })
    });
  }
...
var PATCHERS = [
  ...
  PropertiesPatcher,
  // Added by an LLM agent (P2-15): no `false` holes - filter version-gated
  // patchers at load time so this.patchers has no undefined entries.
  ...!(0, import_obsidian28.requireApiVersion)("1.12.0") ? [BacklinksPatcher] : [],
  OutgoingLinksPatcher,
  // Metadata dependant patchers
  ...(0, import_obsidian28.requireApiVersion)("1.9.0") ? [BasesTableViewPatcher] : [],
...
var CANVAS_EXTENSIONS = [
  ...
  // Added by an LLM agent
  NoteLabelVisibilityCanvasExtension,
  // Added by an LLM agent
  DefaultStyleSizesCanvasExtension,
  // Added by an LLM agent
  NodeOutlineCanvasExtension,
  // Added by an LLM agent
  EdgeOutlineCanvasExtension,
  // Added by an LLM agent
  ...
  // Added by an LLM agent
  // Added by an LLM agent (P2-14): ordering is a declared dependency, not tribal
  // knowledge. EdgeArrowBase measures the final path produced by EdgeStyles, and
  // BezierCurvature scales EdgeArrowBase's control points - keep this order and
  // see getExtensionOrderDependencies() below for the machine-checked version.
  EdgeArrowBaseCanvasExtension,
  // Added by an LLM agent
  BezierCurvatureCanvasExtension,
  // Added by an LLM agent
  ...
  // Added by an LLM agent
  HideNodeContentsCanvasExtension,
  // Added by an LLM agent
  TextWrapAroundCanvasExtension,
  // Added by an LLM agent
  PresentationFilterCanvasExtension
  // Added by an LLM agent
];
var AdvancedCanvasPlugin = class extends import_obsidian28.Plugin {
  async onload() {
    ...
    this.patchers = PATCHERS.map((Patcher2) => {
      try {
        ...
        console.error(`Error initializing patcher ${Patcher2 == null ? void 0 : Patcher2.name}:`, e);
        try {
          new import_obsidian28.Notice(`Advanced Canvas: ${Patcher2 == null ? void 0 : Patcher2.name} failed to patch (Obsidian API changed?). See console.`);
        } catch (e2) {
        }
        return void 0;
      }
    ...
    this.canvasExtensions = CANVAS_EXTENSIONS.map((Extension) => {
      try {
        ...
        console.error(`Error initializing ac-extension ${Extension == null ? void 0 : Extension.name}:`, e);
        return void 0;
      }
    }).filter((e) => !!e);
    this.assertExtensionOrder();
  }
  onunload() {
    var _a, _b;
    for (const extension of (_a = this.canvasExtensions) != null ? _a : []) {
      try {
        extension.dispose();
      } catch (e) {
        console.error(`Error disposing extension ${(_b = extension == null ? void 0 : extension.constructor) == null ? void 0 : _b.name}:`, e);
      }
    }
  ...
  // Added by an LLM agent (P2-14): machine-checked declaration of the
  // order-sensitive extension pairs. onload asserts this after constructing
  // extensions so a future reorder fails loudly instead of silently changing
  // global event order.
  assertExtensionOrder() {
    var _a;
    const order = ((_a = this.canvasExtensions) != null ? _a : []).map((e) => e.constructor.name);
    const mustComeAfter = [
      ["EdgeArrowBaseCanvasExtension", "EdgeStylesExtension"],
      ["BezierCurvatureCanvasExtension", "EdgeArrowBaseCanvasExtension"]
    ];
    for (const [later, earlier] of mustComeAfter) {
      const laterIdx = order.indexOf(later);
      const earlierIdx = order.indexOf(earlier);
      if (laterIdx !== -1 && earlierIdx !== -1 && laterIdx < earlierIdx)
        console.error(`Advanced Canvas: ${later} must be constructed after ${earlier} (see CANVAS_EXTENSIONS order).`);
    }
  }
  getCurrentCanvasView() {
    const canvasView = this.app.workspace.getActiveViewOfType(import_obsidian28.ItemView);
