...
MODIFIED:
  ...
  4. vault.cachedRead: patched to strip triple-quote delimiters (""") from
     custom-extension files when rendering embeds, so they don't clutter
     the ![[note]] preview output.
*/
...
var CustomFileExtensions = class extends import_obsidian3.Plugin {
  ...
  async onload() {
    ...
    this._patchEmbedRead();
  }

  // -------------------------------------------------------------------------
  // _patchEmbedRead
  //
  // Patches vault.cachedRead to strip triple-quote delimiters (""") from
  // custom-extension files when they are read for embed rendering. This
  // prevents """ docstring markers from cluttering ![[note]] previews.
  //
  // Only applies to custom-extension files (e.g. .gd, .txt) registered as
  // the markdown view type — never to actual .md files, so the triple-quote-
  // blocks plugin continues to work normally in regular notes.
  // -------------------------------------------------------------------------
  _patchEmbedRead() {
    const origCachedRead = this.app.vault.cachedRead.bind(this.app.vault);
    this._origCachedRead = origCachedRead;
  ...
  onunload() {
    // Restore vault.cachedRead before anything else.
    if (this._origCachedRead) {
      this.app.vault.cachedRead = this._origCachedRead;
    }
